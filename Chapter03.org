...
#+LATEX_HEADER:\graphicspath{{../SimpleResponsesChapter/gfx/}{../../cnstellate/Responses_CS}}
...
deactivating potassium current \I                                                                                             KA and hyperpolarisation-activated mixed cation
...
model at 20, 30 and 40 dB re $\theta$ was
...
Elevated activity from inputs GLG, TV and DS cell model at 20 dB re $\theta$ (55 dB \SPL) reduced the regularity of
...
\wTVTS
call_opt_paramf(param="w.x[tv][ts]",type="CT2",factor="1000")[:results replace raw] nS).  The GABAergic input weight was very low (\wGLGTS
call_opt_paramf(param="w.x[glg][ts]",type="CT2",factor="1000")[:results replace raw] nS), but the loud input stimulus (70 dB \SPL = 30 dB re $\theta$) would
...
The CV statistics used in the chopper optimisations was the dominant modality in
fitting the cell model and network parameters.
The regularity of inter-spike intervals in \TS cells create its typical chopper
\PSTH and is best highlighted by the time varying CV.
The \ChS model matched the sustained regular firing of the exemplar unit, performing best at 30 dB re $\theta$.
...
Average intracellular voltage contains more information regarding synaptic
...
within the bounds for noise.  The results of the post-stimulus MP offset
...
not be visible at recording sites in the soma. This may be the cause of weaker
optimisation results for adaptation and post-stimulus offset membrane difference
measures. Increasing the morphology complexity of the neural model (i.e. using a
passive dendrite segment) would help to improve the \TS model and improve the
