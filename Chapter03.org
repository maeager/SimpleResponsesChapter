...
Table \ref{tab:Meth:AddDelay} shows the additional delay component for each cell type
calculated from population statistics to ensure the first spike latency
in the models matched the experimental data.  The models were designated
from the basic \citet{RothamanManis:2003b} neural models.  The relative
difference between \DS cell model's mean FSL and the other Type I cell
types can be attributed to the additional \IKLT channels in the Type
I-II Rothman and Manis neural model.  \IKLT is active at \RMP and
decreases the overall input resistance and membrane time constant,
reducing the time to fire an \AP.  \TS cells are designated with the
Type I-t Rothman and Manis model, which contains an additional \IKA
channel current over the Type I-c model, but were not significantly
different from the classic Type I models in \TV and \GLG cell models.
...
#+BEGIN_LaTeX
      \begin{table}[tp]
        \centering
        \caption{Additional delay component of ANF to Cell-types}\label{tab:Meth:AddDelay}
        \begin{tabularx}{0.8\linewidth}{lcccc}\toprule
                        & \HSR \ANF &            \TS             &            \DS             &            \TV             & \GLG \\ %\hline
