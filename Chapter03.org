...
model, and the data upon which it is based.  The level of detail in the neural
models of the \CNSM model includes the complex channel dynamics of a \HH
conductance model.  Three potassium current channels (\IKHT, \IKLT, and \IKA)
and the mixed cation hyperpolarisation current (\Ih) were fit precisely from
\VCN neurons in mice
...
Excitatory \TS cell connections to \TV cell have been shown in histological
...
Ultra-structural labelling of synapses in the \DCN shows \TV cells
are not just inhibited by \DS cells but also from \GABA, glycine and mixed sources
within the \DCN \citep{RubioJuiz:2004,Rubio:2005,Alibardi:2003a,Alibardi:2001}. 


Recurrent connections between \TS cells have been proposed \cite{FerragamoGoldingEtAl:1998a}


Glycinergic inputs to DS cells (from \DS and \TV cells) were excluded from the \CNSM model due to  
less effective than GABA.    

Application of strychnine on guinea pigs \DS cells has little or no effect on their response area \citep{EvansZhao:1998}.


\TV cells receive known inputs from local GABA and mixed GABA/Glycine neurons .

Key examples of these are the \DCN cells connecting with TV cells
\citep{SpirouDavisEtAl:1999,YoungDavis:2002}; 

\DS cells from the contralateral \CN form fast, weak inhibitory connections with
...
GABAergic collaterals of the periolivary region
...
The inclusion of these interactions was beyond
...
THe results are limited in their generalisability to other animal species, other networks


Other studies have used more stringent methods of cell type classifications 

THe basis of the \CNSM model 
Experimental data 
