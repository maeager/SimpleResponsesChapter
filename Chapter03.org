...
the \CNSM model to the verification stimuli.  \yellownote{More text here}
#+BEGIN_LaTeX
  {%
    \begin{figure}[tp!]
      \centering
...
The optimised TV cell model response in Figure \ref{fig:TVRateLevelResult}C show
a poor fit for the control tone rate-level output, especially at high SPL. The
\RMS of the tone rate level response in the experimental control and optimised
\TV cell model was call_TV_RMS_error(errstr="# RL Error") sp/s.  This response was
non-linear and showed stronger inhibitory effects without recovery to a stable
firing rate. The discrepancy is problematic for this particular experimental
data but other Type II DCN units in Figures \ref{fig:SpirouFig1}B and C also
showed strong suppression of \CF tones at high level.  The noise rate level \RMS
error was significantly better at call_TV_RMS_error(errstr="# NRL Error") sp/s, an
important factor in the Type II EIRA response of the TV cell model.  For the
strychnine-affected experimental data and response of the \TV cell model without
\DS cell inputs (Figures \ref{fig:TVRateLevelResult}E and F), the \RMS error for
the tone and noise rate level was 
call_TV_RMS_error(errstr="# RL without DS Error") and 
call_TV_RMS_error(errstr="# NRL without DS Error ") sp/s, respectively.
The model's response was well fit with discrepancy in mid-level tones and a
shift in threshold by 15 dB in noise stimuli.  For the bicuculine-affected
experimental data and equivalent TV cell model (Figures
...
coincidence of onset inhibition dramatically affects the TV cell model where the
activity in the model was predominantly in the second half of the 50 msec
stimulus. In noise, the DS cell model's rate level was slightly higher but the
LSR noise rate level flattened out (Figure \ref{fig:TVRateLevelResult}B) contributing to a low saturation rate in the
TV cell model (Figure \ref{fig:TVRateLevelResult}H).  The final error used in the optimisation routine was a summation
of the \RMS errors (call_TV_RMS_error(errstr="# Final error ") sp/s).
...
fibres (\wLSRTV = call_TVopt_param(param="w.x[lsr][tv]",factor="1000")[:results
replace raw] nS, \wHSRTV =
...
parameter \sDSTV=30). 
Due to the \OnC response properties of \DS cells the inhibition is most
effective at stimulus onset and at higher sound levels.  
The weight of GLG cell
inputs was comparable to \DS cell's synaptic weight (\wGLGTV =
...
suggest a continuous monotonic inhibition that regulates the activity



The discrepancies in the rate level optimisation present more limitations of the
CNSM model and the optimisation process.  Acoustic threshold and the gradient of
the rate-level response in each of input cell types play a critical role in the
output responses of TV cells, which are considered as simple integrators
\citep{ZhangOertel:1993b}.  The cytoarchitecture of TV cells and their synaptic
inputs may be more complex than originally proposed
\citep{Rubio:2005,Rubio:2006,Alibardi:2006,MunirathinamOstapoffEtAl:2004}.  The
\TV cell receives more complex inputs in the deep layer of the DCN that are not
accounted for in the model.  The complexities of the DCN circuit are similar to
...
responses. This may not be the best fit or design but the responses in the control
scenario, with all inputs simulated, was a satisfactory demonstration of a TV cell
model with all the features of a Type II EIRA DCN unit: no spontaneous activity;
non-linear rate level response to tones and no response to noise.
...
optimisations, were tested using tones and noise stimuli across the whole \CNSM
network.  Figure \ref{fig:TVVerificationPSTH}A shows the PSTH response of the
central \TV unit (\CF 4.5 kHz) at 40 dB \SPL is classified as \OnL.  The minimum
first spikes occur at 4.3 msec after the stimulus onset (similar to the
un-optimised \TV cell model's mean \FSL in Section [[Synaptic Delays]]).  The
...
\ref{fig:TVVerificationPSTH}B).  In noise, the PSTH of the TV cell model had
...
of fitting to particular cell responses in a noisy \BNN model. The complications were
...
the exemplar unit at each of the stimulus levels.  The post-offset hyperpolarisation results of
the exemplar \ChTone unit were not consistent with the inhibitory behaviour of
\TS cells, especially \ChTone units. The lack of hyperpolarisation in /in vivo/ recordings

This statistic was ignored in the optimisation procedure.
...
\yellownote{TODO: Discuss the table.}

Table \ref{tab:Finalparams} show the aggregated optimised parameter values of
the synaptic weight and number of connections in the CNSM model.  One important
point for the table is that the role of \LSR fibres in the CNSM model appears to
have a stronger effect than \HSR\space \ANFs in each of the \CN units.  Their
large dynamic range was beneficial to the GLG and DS cell models that were
fitted to experimental data with monotonic rate level responses. For TS and TV
cell models, LSR ANFs are the only excitatory source that counters the
increasing inhibitory sources, GLG and DS cells, at high SPL. Further investigation
...
|      | \GLG                | \DS                   | \TV                  | \ChS                   | \ChTone               | \ChTtwo               |
| --   |                     |                       |                      |                        |                       | ----                  |
| \HSR | 0.0487$\,\times\,$1 | 0.1672$\,\times\,$125 | 0.6451$\,\times\,$10 | 0.5056$\,\times\,$30   | 0.9834$\,\times\,$25  | 1.045$\,\times\,$25   |
| \LSR | 0.5166$\,\times\,$1 | 11.03$\,\times\,$84   | 4.172$\,\times\,$20  | 1.74$\,\times\,$30     | 1.216$\,\times\,$25   | 1.149$\,\times\,$25   |
| \GLG | 0$\,\times\,$0      | 0.5315$\,\times\,$5   | 1.586$\,\times\,$10  | 0.008953$\,\times\,$20 | 0.248$\,\times\,$20   | 0.3212$\,\times\,$20  |
| \DS  | 0$\,\times\,$0      | 0$\,\times\,$0        | 3.586$\,\times\,$30  | 0.7337$\,\times\,$20   | 0.5615$\,\times\,$25  | 0.4348$\,\times\,$20  |
| \TV  | 0$\,\times\,$0      | 0$\,\times\,$0        | 0$\,\times\,$0       | 0.07319$\,\times\,$20  | 0.02007$\,\times\,$20 | 0.06719$\,\times\,$20 |
...
#+RESULTS:
|   | 6.71923e-05 |           0 |           0 |      0 |
|   | 0.000434838 |  0.00358648 |           0 |      0 |
|   | 0.000321159 |  0.00158648 | 0.000531549 |      0 |
|   |  0.00104469 | 0.000645083 | 0.000167221 | 0.0487 |
|   |  0.00114944 |   0.0041716 |     0.01103 | 0.5166 |
