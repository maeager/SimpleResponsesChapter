...
#+END_QUOTE
#+LaTeX:\vspace{-4ex}{\hfill S.~\textit{\citet[~p.~411]{Shamma:1998}}}\\ \vspace{4ex}
...
the \CNSM model, discussed in Section \ref{sec:Meth:cell-models}, remain
relatively fixed. The connectivity-based parameters, introduced in
Section \ref{sec:Ch2:Parameters}, are the main focus of this chapter.
...
input \AN model (chosen from the Bruce or Zilany model) was passed through the
...
Section \ref{sec:Meth:cell-models}.
...
in  Section [[sec:Ch2:SynapseModels]].
...
\TS cells are designed with the Type I-t \RM model, which contains an
...
without the use of the function's gradient.  The fitness function
...
plus noise stimuli. Figures \ref{fig:GolgiVerificationPSTH} shows a
...
#+BEGIN_LaTeX
    \begin{figure}[t!]
      \centering
  \includegraphics[width=0.4\textwidth]{../SimpleResponsesChapter/gfx/RothmanManisFig2D.png}
  %    \includegraphics[width=0.\textwidth]{../figures/Oertel_Fujino.pdf}
  \includegraphics[width=0.8\textwidth]{../figures/Oertel90DS_bottom.png}
      \caption[Intracellular membrane voltage response of DS cells to depolarising and hyperpolarising current]{Intracellular membrane voltage response of DS cells to depolarising and hyperpolarising current. (Top row) Simulated current clamp response in a Type I-II RM model \citep[reproduced~from]{RothmanManis:2003b}. (Bottom row) One DS cell's response to different levels of current injection \citep[reproduced~from][]{OertelWuEtAl:1990}. %E-G Three DS units with prominent hyperpolarising sag \citep[reproduced~from][]{FujinoOertel:2001}.
  }
...
\LSR fibres) and \GLG cells.  The \CNSM model assumed that \GABA-ergic
...
Table \{tab:DScellModelSummay}E describes the fitness function stimulus as
...
The following section introduces, defines and optimises the neural model of Tuberculoventral cells of the \DCN.
...
simulations using a 70 dB \SPL not-noise stimulus.  The optimisation
...
The results in the \TV cell model not optimisation show the dominance of
...
MATLAB/GNU Octave]] using Chebychev II filters with a sampling rate of 100
...
#+BEGIN_LaTeX
    \begin{figure}[thp]
      \centering
    \includegraphics{../SimpleResponsesChapter/gfx/PaoliniCV.pdf}
...
\yellownote{ complete synpatic connections to TS cell section}
...
the \CV calculations.  The intracellular voltage statistics, introduced by
\citet{PaoliniClareyEtAl:2005}, included the onset ratio, adaptation shift and
offset shift.  The \IV measures highlight inhibition in \TS cells and were
designed to be independent of \RMP and \FSL. The effective stimulus onset was
determined as 2.5 ms prior to the peak in average membrane voltage

The onset ratio measure was calculated as the ratio in peak onset depolarisation (sum over the
first 5 ms) relative to the sustained depolarisation (sum measured between 20 to 25 ms).
#+BEGIN_LaTeX
  \begin{equation}
...
The adaptation shift measurement calculates the drop in depolarisation
at the end of the stimulus compared to the stable point 20 ms after the
onset
#+BEGIN_LaTeX
  \begin{equation}
...
factors, but is mostly due to reduction excitation from ANF inputs which
is caused in turn by adaptation at the IHC-AN synapse.
...
Negative values indicate post-stimulus hyperpolarisation which is caused by
 reduction of excitation and/or the presence of inhibition.
...
#+BEGIN_LaTeX
    \begin{figure}[thp]
      \centering
    \includegraphics{../SimpleResponsesChapter/gfx/TStellate/PaoliniIV.pdf}
  B. Average membrane voltage hyperpolarisation after stimulus offset (Equation \ref{eq:IVstats-Offset}) at different sound levels relative to threshold. Transient chopper data was gathered from Figure 6B in \citet{PaoliniClareyEtAl:2005}.
    \caption[IV categorisation in chopper units]{A.  Adaptation shift (Equation \ref{eq:IVstats-Adap}) at different sound levels (relative to threshold). Transient chopper data, representing average within ChT$_{1}$ and ChT$_{2}$ groups, was gathered from Figure 6A in \citet{PaoliniClareyEtAl:2005}. 
  The average post-tone hyperpolarisation (offset shift) in ChS units at 30 dB re threshold is 1.6 mV.
  ChS unit 01-859-013  (CF=3.9 kHz, Th 25 dB) is used for the ChS offset shift data at 20 and 40 dB re threshold \citep[reproduced from Figure~3][]{PaoliniClareyEtAl:2005}.
  }
...
\CF tone bursts at three \SPLs above threshold (20, 30 and 40 dB) were used in
all three \TS cell model optimisation.  The threshold was assumed to be 10 dB
SPL and the CF of the unit was 9.1 kHz.  Figure \ref{fig:PaoliniIVdata} shows
the results of the chopper sub-groups for adaptation shift (see Equation
\ref{eq:IVstats-Adap}) and average post-tone hyperpolarisation (Equation
\ref{eq:IVstats-Offset}). These data points, along with the CV data points in
Figure \ref{fig:PaoliniCV} at 30 db re threshold, were used in the fitness
function as a root mean squared comparison against the \TS cell model.
...
points.  Average \ChS population IV data was limited to one data point at
offset shift at 30 dB re threshold (1.6\pm1.0 mV) significant from
membrane variations at rest.  The unfilled triangles in Figure
\ref{fig:PaoliniIVdata} were measured from the average membrane voltage trace of
unit 01-859-013 \citep[Figure~3~in][]{PaoliniClareyEtAl:2005}.  Adaptation shift in
\ChT1 and \ChT2 units increased with increasing \SPL (Figure
\ref{fig:PaoliniIVdata}A), reaching a peak at 30 dB abovethreshold.  \ChTtwo
units consistently had a stronger inhibitory influence (adaptation and
offset shifts) compared to \ChTone for all \SPLs above threshold.
...
\citep{PaoliniClareyEtAl:2005}. Due to time constraints, the optimisation
of
...
for the \ChS \TS cell model.  
This unit had a depolarisation threshold at 10 dB and a
firing rate saturation at 75 dB \SPL.  

The stimulus for the optimisation
...
#+BEGIN_LaTeX
  ...
  \caption[Sustained chopper optimisation results]{Optimisation results for the
    ...
    D. Measures of membrane voltage including a) onset ratio (Equation
    \ref{eq:IVstats-Onset}), b) adaptation of sustained depolarisation (Equation
    \ref{eq:IVstats-Adap}), and c) post-tone hyperpolarisation (Equation
    \ref{eq:IVstats-Offset}).  The RMP, calculated from -10 to 0 ms, for the
...
\yellownote{Increase gKA helps increase potassium conductance at rest}
...
#+BEGIN_SRC sh :exports none
  grep -A 14 'at spl '$SPLreTh''  ~/Work/cnstellate/TStellate2_CS/TStellate.Fit.dat | tail -6  | awk 'BEGIN{sum=0;} {sum+=$3;} END{print sum/5.0}'
#+END_SRC

The mean rate for the optimised \ChS unit at 20, 30 and 40 dB re threshold was
call_CS_rate(SPLreTh=30), 
call_CS_rate(SPLreTh=40) and 
call_CS_rate(SPLreTh=50) 
spikes per second, respectively.
...
The \ChTone unit chosen for the optimisation was unit 01-857-007 
...
\ChTone units.  The stimulus for the optimisation routine was an 85 dB 8.2
kHz pure tone and the exemplar \TS cell model had CF 8.27 kHz.

The optimisation results of \ChTone are shown in Figure
\ref{fig:CT1results} with best parameters as shown in Table \ref{tab:TSCellResults}.
...
#+BEGIN_LaTeX
  \begin{figure}[t!]
    \centering
  \resizebox{0.95\textwidth}{!}{\includegraphics{../SimpleResponsesChapter/gfx/TStellate_CT1/multiplotIVCV4}}
...
#+name: CT1_rate
#+BEGIN_SRC sh  :exports none 
  grep -A 14 'at spl '$SPLreTh''  ~/Work/cnstellate/TStellate2_CT1/TStellate.Fit.dat | tail -6 | awk ' {print $3}' | meanminmax | awk '{printf("%.1f",$3)}'
#+END_SRC


The mean rate for the optimised \ChT1 unit at 20, 30 and 40 dB re threshold was
call_CT1_rate(SPLreTh=60), 
call_CT1_rate(SPLreTh=70) and 
call_CT1_rate(SPLreTh=80)
spikes per second, respectively.
...
The \ChTtwo unit optimisation used average population data for CV data and 
unit 01-305-014 (CF 12.4
kHz \citep*{PaoliniClareyEtAl:2005}) for IV measure data. The reference
unit had a low depolarising threshold (5 dB \SPL) so the stimulus was
set to 35 dB \SPL; a significant difference from the \ChT1 optimisation
unit that was optimised using an 85 dB \SPL tone. The main difference
between the transient chopper types is reflected in their \CV values
after 10 ms, with the \ChT2 type becoming irregular (i.e. having a CV
value over 0.3) and maintaining that regularity for the remainder of the
stimulus.  \ChT2 has a higher reduction in onset depolarization (90.2\%)
compared to ChT1, and post-tone inhibition is greater (8.82 mV)
...
#+BEGIN_LaTeX
  ...
  \resizebox{0.95\textwidth}{!}{\includegraphics{../SimpleResponsesChapter/gfx/TStellate_CT2/multiplotIVCV3}}
  \caption[Transient chopper 2 optimisation results]{TS cell model
    ...
    \citep{PaoliniClareyEtAl:2005} that reduced the effectiveness of \DS cell
...
Dendritic filtering  was not included in \DS and \TV cell models. 
