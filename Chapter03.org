...
Mean relative error (MRE) was also
 used to ensure there was no bias between data points with large variation
in magnitude. The equation for MRE using rate level responses is
#+BEGIN_LaTeX
\begin{equation} \label{eq:Ch3:RMS}
...
58.59\%.  The poor fit of the tone rate level response (73.36\% relative
...
lower mean relative error (37.25\%) across the sound levels; however the DS cell
