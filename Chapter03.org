...
responses of the optimised DS cell model and the reference DS unit.  At 50 repetitions per stimulus,
the best parameters produced a combined \RMS error of 48.88 spikes per second
(tone RMS 61.6 sp/s, noise RMS 25.14 sp/s).
...
levels, especially around the acoustic threshold.  The optimisation routine
...
(optimised \DS cell model relative to the reference DS cell) of the tone and
noise rate level responses in Figure \ref{fig:DSRateLevelResult} was
...
mammals \citep{Schmiedt:1989,HeilNeubauerEtAl:2007,Liberman:1993,Liberman:1978}.
