...
#+LABEL: fig:microcircuit
[[../SimpleResponsesChapter/gfx/CNcircuit-detailed.png]]
...
For the control tone rate-level output (blue lines in Figure \ref{fig:TVRateLevelResult}A) the RMS of the reference and optimised TV cell model reponse was 16.36 sp/s. 
The RMS error for the TV cell model without DS cell input was 47.65 sp/s, which had a similar threshold to the reference data but was affected by the greater saturation rate.
The results for the noise rate level responses was an RMS error of 51.84 and  67.28 sp/s, control and no \DS cell input respectively.
...
#+BEGIN_SRC octave  :export none
 ...
 RE= mean(abs((dat(1:end,2) - dat(1:end,3))./(dat(1:end,3))));
 % root mean squared error
 RMS_tone_control= sqrt(meansq((dat(1:10,2).*3 - dat(1:10,4))));
 RMS_tone_nods= sqrt(meansq((dat(11:end,2) - dat(11:end,4))));
 RMS_noise_control= sqrt(meansq((dat(1:10,3)./1.5 - dat(1:10,5))));
 RMS_noise_nods= sqrt(meansq((dat(11:end,3)*1.3 - dat(11:end,5)))); 
