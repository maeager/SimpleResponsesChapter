...
high \CF \DS cells ($2.8 \pm 0.09$ ms) is precise and faster than other stellate
...
averaging intracellular membrane responses to acoustic stimuli to determine
stochastic excitatory and inhibitory inputs was first used in the chopper units
of gerbils \citep{OstapoffFengEtAl:1994,FengKuwadaEtAl:1994}.
...
and {21.6 \pm 11.3} \um in cats \citep{ReddCahillEtAl:2002}.  The variation is
...
\CF regions (high \CF {14.5 \pm 2.0} \um, low \CF {17.6 \pm 2.7} \um
...
(mice at room temperature) 9.1 \pm 4.5 \citep{ManisMarx:1991}; (rat at room
temperature) 6.9 \pm 3 msec \citep{IsaacsonWalmsley:1995}; and 8.8 \pm 1.4 in dogs
...
deactivating potassium current \KA and hyperpolarisation-activated mixed cation
...
the \TS cells, its major output.  The \TS cell optimisation study attempted to
constrain the many network parameters using intricate intracellular and spiking
statistics.
# translatating intricate intracellular and spiking statistics into a single objective value.

\yellownote{CV statistics results in TS cells}
...
Average intracellular voltage contains more
information regarding synaptic inputs and sub-threshold membrane dynamics than
mere spikes alone.  The \AIV statistics of \citep{PaoliniClareyEtAl:2005} were
modified to 5 ms sampling to allow for less repetitions in the optimisation
routines.  For both the offset ratio and adaptation MP difference measures, the
optimal neurons' responses were within the bounds for noise.  The results of the
post-stimulus MP offset difference show the limitations of comparing real and
simulated intracellular recordings.  In the single-compartment neural model, the
...
and optimisation results.  Accurate replication of realistic responses to
acoustic stimuli in the CNSM model is dependent on the capabilities of the input
AN model. The background and driving inputs to the network set the environmental
conditions for genuine realistic models to be compared with experimental animal
data under the same conditions and inputs. This limitation was overcome by using
a phenomenologically-realistic \AN model, the most detailed available at the
time of the model's development.
...
# All optimisation    procedures used experimental units that were positioned near the middle of the CNSM model.


The connectivity properties (number, weight, spread and delay) were uniform
across frequency channels in the CNSM model. This uniformity excludes natural
variations in synaptic connections in order to minimise the number of network
parameters, and reduces the variation of output responses within cell types.
This approach is justifiable for synaptic connections to the GLG, DS and TV
cells in the CNSM model because of their homogeneity. In light of the three
different physiological output responses of \TS cells, unique parameters were
used to separate each type.
...
namely the addition of new cell types was beyond the scope of the model.
...
(\Ih) that were fitted precisely from intracellular recordings in mice \VCN
...
thesis, and requires investigation in future research.


Finally, the generalisability of the CNSM model and the optimisation results is
...
# -------
\yellownote{implication paragraph}
...
Understanding the limitations of experimental data facilitates setting out
constraints in fitness functions.
...
Design and planning of sequential optimisation stages, ensures each stage does not affect previous optimisation results.
# Feed-forward
One issue is whether to optimise cell types based on
population responses or exemplar unit responses.
Fitness functions should be targeted toward equalisation of all available information.
This was done in this chapter simple stimuli (tones, noise and clicks) over the full range of sound levels as observed in experiments.
Another issue is ensuring equal weighting of information gathered from different modalities (e.g. AIV and CV statistics in the \TS cell
optimsiation) so that each metric contributes equally to the fitness function.


# ----------
The development and optimisation of the \CNSM model demonstrates the importance
of close attention to the specifics properties of data used in \BNN models.
Experimental devices and methods differ across laboratories, across different species and
...
# ------
Sources of noise in a \BNN model have critical implications for the network in
its output and in the optimisation of the model's parameters.  Noise is present
in \BNNs at the level of ion channels, in synaptic jitter, in local variation of
synaptic strengths, and in variation of synaptic properties across nuclei.  The
sources of noise in the CNSM model are the spike generators of the ANF and GLG
cell models and in the allocation scheme of number of connections between cell
types (generally a Gaussian distribution except for ANF inputs to DS cells).
Also, output responses of neurons need to be averaged over multiple repetitions
of the same stimuli to encompass stochasticity of the neuron's inputs.
Optimisation of synaptic parameters also requires a gradient in the search
...
\BNN models and their optimisation.
...
\yellownote{TODO: More editing in chapter conclusion.  Is one paragraph enough?}
