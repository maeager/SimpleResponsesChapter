#+TITLE: Optimisation of the Cochlear Nucleus Stellate Network Model: Sequential Parameter Fitting of Synaptic Variables Using Simple Acoustic Responses
#+DATE:
...
#+BEGIN_LaTeX
    ...
    % \includegraphics{}\hfill}\\
      {\figfont{A}\hspace{0.5\textwidth}\figfont{B}\hfill}\\
      {\hfill%
        \resizebox{0.25\textwidth}{!}{\includegraphics{../../cnstellate/ResponsesNoComp/RateLevel/psthsingle5090-3.pdf}}%
        \resizebox{0.5\textwidth}{!}{\includegraphics{../SimpleResponsesChapter/gfx/GhoshalKim_PSTHs.png}}%
        \hfill%
      }\\
...
** Discussion and Limitations of the GLG cell model

The GLG cell model is an important cell type the \CNSM model 
due to being the only GABAergic interneuron.


The neural model and AN inputs  

The decision to use a filter-based point-process neural model rather than a
\HH-like neural model was based on the limited intracellular evidence for the latter in the
literature \citep[see for example][]{FerragamoGoldingEtAl:1998}.
# While we didn't use the \RM neural model in this chapter, 
# Chapter \ref{sec:GAChapter} uses a classic type 1 \RM neural model to model the \GLG cells.
Computational speed and efficiency was another contributing factor for using the
filter model.  The GLG cell model's instantaneous rate responses to stimuli were
saved, similarly to ANF responses, so that they could be reloaded at simulation
time then spikes can easily be generated at initialisation.
 
Synaptic inputs to GLG cells, other than HSR and LSR ANFs, were not included in the GLG cell model 
due to limited histological and electro-physiological evidence.

Monotonic rate level responses of the GLG cell model are accurate enough to
proceed with the remainder of the CNSM model without including more detail to
the cell model or adding new cell types and connections to the model.


\yellownote{Limitations: 
1. Point neuron model rather than HH model, 
2. inhibitory inputs (other Golgi, glycinergic from DS)
3. Mossy fibres (type 2 ANFs, granule cells) NMDA synapses, insufficient reuptake of glutamate in GCD
4. Efferent feedback from higher centres and non-auditory inputs to the GCD.
} 

# Justification: its model and connections are good enough to proceed.  
...
The level of detail and complexity in the neural models of the \CNSM model was a
limitation that was properly constrained.  The \RM neural models used in the DS,
TV and TS cell mdoels included the complex channel dynamics of a \HH conductance
model.  Three potassium current channels (\IKHT, \IKLT, and \IKA) and the mixed
cation hyperpolarisation current (\Ih) were fit precisely from \VCN neurons in
mice \citep{RothmanManis:2003,RothmanManis:2003a,RothmanManis:2003b}.  More
complex multi-compartment conductance model could have been chosen to include
dendritic filtering, especially in \DS and \TS cells.  Ball-and-stick neural
models already existing in stellate models
\citep{WangSachs:1995,WiegrebeMeddis:2004}, however simple point neural models
are quite cabable of demonstrating complex stellate behaviour
\citep{KalluriDelgutte:2003}. The single compartment, conductance model with
unique channels developed by \citep{RothmanManis:2003b} was a reasonable choice
for a realistic CNSM model.  The novel filter-based neural model developed for
the \GLG cell model was chosen due to the evidence regarding intracellular
properties of Golgi cells were limited to just one study
\citep{FerragamoGoldingEtAl:1998}. 
