...
better at call_TV_RMS_error(errstr="# NRL Error") sp/s, an important factor in the
...
noise rate level was call_TV_RMS_error(errstr="# RL without DS Error") and
...
call_TV_RMS_error(errstr="# RL wo GLG Error ") and call_TV_RMS_error(errstr="# NRL
wo GLG Error ") sp/s, for tone and noise rate level, respectively.  The DS cell
...
was a summation of the \RMS errors (call_TV_RMS_error(errstr="# Final error ")
sp/s).
...
#+begin_src latex :exports results :results latex  :noweb yes
  \begin{table}[htb!]
    \centering
    \footnotesize
    \caption[Optimised Synaptic weight and number of connections in the CNSM model]{Optimised Synaptic weight and number of connections in the CNSM model.  The units of synaptic weight are in nanoSeimens with the exception of GLG cell inputs that are unitless.}
