...
properties of a detailed neural model of the cochlear nucleus, a realistic and
phenomenologically sound auditory model is needed to represent sounds and
transformations that occur in the central auditory system.  The input to the
\CNSM model, the auditory nerve fibres, is discussed in the next section. To
further fit the parameters of the CN model, a simple and sequential method was
...
\GLG cell model in the \CNSM model then optimises the model
parameters based on rate level information.
...
dense, axonal plexus, limited to the plane of the \GCD,
...
In a single study in mice, intracellular recordings of \GLG cells
...
known source of \GABA intrinsic to the \VCN is the \GLG cells of the
...
\GLG cell model were as follows:
 1. \GLG cells are classic integrator neurons, as shown by 
 ...
 4. The low threshold in \GLG cells can\-not be solely due to \LSR
...
The optimal parameters of the \GLG cell model had a fitness score of
...
range in the \GLG cell model. Equally, the \HSR fibres were necessary
...
organisation in the \GCD.  The dendritic widths in \GLG cells are
...
#   %     \label{Ch3:fig:GolgiRL}}
#   % \includegraphics[width=0.8\textwidth,angle=-90]{GolgiRateLevel2}\\
#     %   \caption{Optimisation Results for \GLG Model using Rate Level data
...
\citep{FerragamoGoldingEtAl:1998a}.  \GLG cells are the only
...
project into the \GCD \citep{ArnottWallaceEtAl:2004}.  \GLG cells'
