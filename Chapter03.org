...
#+BEGIN_LaTeX
    ...
    \includegraphics{../SimpleResponsesChapter/gfx/PaoliniIV.pdf}
    \caption[Categorisation in chopper units]{A. Regularity in chopper units measured using 10 ms interval CV at 30 dB relative to acoustic threshold. Data represents mean and standard deviation in units in each category. Data reproduced from Figure 2 in \citet{PaoliniClareyEtAl:2005}. B.  Average membrane adaptation (Equation \ref{eq:IVstats-Adap}) at different sound levels (relative to threshold). Transient chopper data, representing average within ChT$_{1}$ and ChT$_{2}$ groups, was gathered from Figure 6A in \citet{PaoliniClareyEtAl:2005}. 
  C. Average membrane voltage hyperpolarisation after stimulus offset (Equation \ref{eq:IVstats-Offset}) at different sound levels relative to threshold. Transient chopper data was gathered from Figure 6B in \citet{PaoliniClareyEtAl:2005}.
  The average hyperpolarisation offset in ChS units at 30 dB re threshold is 1.6 mV.
  ChS unit 01-859-013  (CF=3.9 kHz, Th 25 dB) is used for the ChS hyperpolarisation data at 20 and 40 dB re threshold \citep[reproduced from Figure 3][]{PaoliniClareyEtAl:2005}.
  }
...
\yellownote{TODO: Synaptic inputs known and unknown, included and not included in model.  Previous work include and excludes}

Figure \ref{fig:TSinputs} shows the expected response of a \TS
cell to individual connections from different cells in the \CNSM model.
The membrane parameters for the single compartment \TS cell model were
default except for sodium conductance, which was set to zero.  In this
...
The optimisation fitness function, $F$, of the \TS cell model calculated
the relative error between a combination of the intracellular and spike
statistics against the experimental data by
...
\citep{Pfeiffer:1966a,BlackburnSachs:1989,YoungRobertEtAl:1988}.  Spike
times for a single repetition were ignored for \CV calculations without
the first three regular spikes or spikes occurring before the stimulus
onset.  \CV of the inter-spike intervals was calculated over 10 ms
windows \citep[as used by][]{PaoliniClareyEtAl:2005}.

The intracellular voltage statistics, introduced by
\citet{PaoliniClareyEtAl:2005}, included the onset ratio, adaptation
shift and offset shift.  The measures of membrane voltage highlight
inhibition in \TS cells and were designed to be independent of \RMP. The
onset ratio measures the change in peak onset depolarisation over the
first 5 ms relative to sustained depolarisation after 20 ms.
#+BEGIN_LaTeX
  \begin{equation}
...
drop in depolarisation. Adaptation is caused by a combination of
...
The offset shift measures the effects of sustained inhibition and
recovery of excitation 10 ms after the stimulus offset
#+BEGIN_LaTeX
  \begin{equation}
  \label{eq:IVstats-Offset} \textrm{Offset} = \bar{\mathbf{v}}|_{t=-10}^{t=0}\quad - \quad\bar{\mathbf{v}}|_{t=60}^{t=61}. 
  \end{equation}
#+END_LaTeX
Negative values indicate hyperpolarisation which is caused by reduction
or absence of post-stimulus excitation and/or the presence of
inhibition.
...
#+BEGIN_LaTeX
    \begin{figure}[thp]
      \centering
    \includegraphics{../SimpleResponsesChapter/gfx/PaoliniIV.pdf}
    \caption[IV categorisation in chopper units]{A.  Average membrane adaptation (Equation \ref{eq:IVstats-Adap}) at different sound levels (relative to threshold). Transient chopper data, representing average within ChT$_{1}$ and ChT$_{2}$ groups, was gathered from Figure 6A in \citet{PaoliniClareyEtAl:2005}. 
...
\yellownote{Explain new fitness functions}
...
#+BEGIN_LaTeX
  \begin{figure}[thb]
    \centering%
  \resizebox{0.95\textwidth}{!}{\includegraphics{../SimpleResponsesChapter/gfx/TStellate_CS/multiplotIVCV3}}
...
The \ChTone unit chosen for the optimisation was unit 01-857-007, CF=8.2
...
#+BEGIN_LaTeX
  ...
  \resizebox{0.95\textwidth}{!}{\includegraphics{../SimpleResponsesChapter/gfx/TStellate_CT1/multiplotIVCV3}}
...
The \ChTtwo \TS cell model optimisation used unit 01-305-014 (CF 12.4
kHz \citep*{PaoliniClareyEtAl:2005}) as its exemplar cell. The reference
