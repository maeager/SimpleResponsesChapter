...
call_opt_paramf(param="w.x[lsr][ts]",type="CS",factor="1000") nS, \wHSRTS =
call_opt_paramf(param="w.x[lsr][ts]",type="CS",factor="1000") nS) and very low
inhibitory strength from all three interneurons ( \wDSTS =
call_opt_paramf(param="w.x[ds][ts]",type="CS",factor="1000") nS, \wTVTS =
call_opt_paramf(param="w.x[tv][ts]",type="CS",factor="1000") nS, \wGLGTS =
call_opt_paramf(param="w.x[glg][ts]",type="CS",factor="1000") nS). The low input
...
call_param_ts_gleak(type="CS") \Scmsq ) from the initial value (4.907e-04 \Scmsq),
...
call_CS_rate(SPLreTh=30), call_CS_rate(SPLreTh=40) and call_CS_rate(SPLreTh=50)
...
AIV measures used in the optimisation.  Early attempts to use the AIV measures,
which used 1 ms windows, were unable to fit the model to the experimental data.
The fluctuations in the \TS cell model's AIV traces, mainly due to \AP
artifacts, were smoothed out by increasing the sampling window to 5 ms.  The
latency of the depolarisation peak was used to temporally shift the model's AIV
trace for a stronger comparison with the exemplar unit.  The onset ratio and the
adaptation MP difference were only measured for the 30 dB re $\theta$ due to
...
mV for the model and 0.94 mV for the exemplar unit.  For both measures, the
differences are within the bounds for noise.  The results of the post-stimulus
MP offset difference show the limitations of comparing real and simulated
intracellular recordings.  In the single-compartment neural model, the
depression and recovery at the end of a stimulus is due to the recovery
properties of the \ANF model and inhibition from \IPSPs with longer time
constants than the \EPSPs of \ANFs. The experimental recordings in the exemplar
\TS cell have a high \RMP and minimal post-tone offset.  \TS cells of mammals
are known to have very few somatic contacts
...
call_opt_paramf(param="w.x[lsr][ts]",type="CT1",factor="1000") nS, \wHSRTS
call_opt_paramf(param="w.x[hsr][ts]",type="CT1",factor="1000") nS) were balanced
with moderate to low glycinergic synaptic input from \DS and \TV cells (\wDSTS
call_opt_paramf(param="w.x[ds][ts]",type="CT1",factor="1000") nS, \wTVTS
call_opt_paramf(param="w.x[tv][ts]",type="CT1",factor="1000") nS).  The GABAergic
input weight was very low (\wGLGTS
call_opt_paramf(param="w.x[glg][ts]",type="CT1",factor="1000") nS), but the loud
...
\ChT1 model at 20, 30 and 40 dB re $\theta$ was
call_CT1_rate(SPLreTh=30),
call_CT1_rate(SPLreTh=40) and
call_CT1_rate(SPLreTh=50) spikes per second, respectively.
This means the optimisation stimulus levels range are in the saturation region of the cell's rate level curve and not in the expected DR region.
...
remainder of the stimulus.  The combined RMS error of the CV data was 0.064.  The CV at 30 dB re $\theta$ was closest fit to the
