...
#+BEGIN_SRC sh :results replace raw
 par=`echo "$param" | sed -e 's/\([^\\]\)\[/\1\\\\[/g' -e 's/\([^\\]\)\]/\1\\\\]/g'`
 grep ${par} ../SimpleResponsesChapter/gfx/TStellate2_${type}/TStellate.Fit.dat | awk -F' ' '{$3=sprintf("%.4g",$3);print $3}'
#+END_SRC
...
of the optimised \ChTtwo unit was call_opt_error(type="CT2")[:results replace raw],
...
The \ChS model matched the sustained regular firing of the exemplar unit, performing best at 30 dB re $\theta$ with RMS of 0.034.
The \ChTone model achieved the exact same RMS difference
