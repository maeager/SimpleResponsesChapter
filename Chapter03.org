...
[AIV introductory sentence] Average intracellular voltage contains more
...
A number of limitations are common to \BNN models.  Firstly, the level of
...
which set the network dynamics, are often hand-tuned or guessed based on evidential support. Finally, the
...
owes a large part to the input of a phenomenologically-realistic \AN model.  The
ability to model two types of \ANFs was critical for reproducing selective
synaptic strength differences seen in each CN cell type.  The level of detail in
the neural models of the \CNSM model includes the complex channel dynamics of a
\HH conductance model.  Three potassium current channels (\IKHT, \IKLT, and
\IKA) and the mixed cation hyperpolarisation current (\Ih) were fit precisely
from \VCN neurons in mice
\citep{RothmanManis:2003,RothmanManis:2003a,RothmanManis:2003b}. Ball-and-stick
neural models already existing in stellate models to include dendritic filtering
\citep{WangSachs:1995,WiegrebeMeddis:2004}, and used to investigate selective
filtering of \ANFs \cite{LaiWinslowEtAl:1994a}.  More complex multi-compartment
conductance model could have been chosen, especially in \TS, \DS and \TV cells,
...
Sequential optimisation  dependent on each stage not affecting previous optimisation results.

Feed-forward 

population versus single unit responses

Weighting of different modalities : ensuring each optimisation cost function 
incorporates information from each metric equally.



Experimental methods differ across laboratories, across different species and technologies change over time.
Understanding these difference is important when deciding upon which data to use in optimisation.
Cochlear frequency mapping and acoustic threshold different for each species \cite{HeffnerKoayEtAl:2001,HeffnerHeffner:1985}

Acoustic calibration 

Choice of anaesthesia can change the response properties neurons, especially in \CN units \citep{RhodeKettner:1987,AndersonYoung:2004}. 
...
The results are limited in their generalisability to other animal species.
Species generalisation occurs due to scarcity of experimental data in the target
species, contradictory evidence between species, high quality data in species
distant to the target species.  Neural models were chosen from rodents
\citep{RothmanManis:2003,RothmanManis:2003a,RothmanManis:2003b} and matched to
current injection properties /in vitro/
...
Physiological data used in the optimisation procedures used experiments of small
mammals (\citet[cat][]{GhoshalKim:1997}, \citep[guinea
...
exception of the TS cell optimisation (\citet[rat][]{PaoliniClareyEtAl:2005}).


Sources of noise in a \BNN model have critical implications for 

Optimisation of feedback synaptic connections were 

Constraints of experimental data sets used in optimisation routines


Other studies have used more stringent methods of cell type classifications 


Overfitting parameters to noise 
exemplar units
