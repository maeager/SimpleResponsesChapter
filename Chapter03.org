...
needs to be developed in sequential stages.  The
...
cell model in the \CNSM model, discussed in Section \ref{sec:Meth:cell-models}, remain relatively fixed. The connectivity-based
parameters, introduced in Section \ref{sec:Ch2:Parameters}, are the main focus of this chapter.  The parameters in the
...
The next part of this chapter introduces the specific attributes of the Golgi (GLG) cell model in the \CNSM model.
...
\ref{sec:MethodsChapter} (Section [[Connectivity%20and%20Topology%20in%20Neural%20Microcircuits][Connectivity and Topology in Neural Microcircuits]]).
...
The next section introduces the major wide-band onset inhibitor, the \DS cell model, of the \CNSM model.
...
shows two \OnC unit rate level responses
to \CF tones and broadband noise  \citep{ArnottWallaceEtAl:2004}.  Both units have higher acoustic thresholds to broadband noise than pure tones and higher maximum rate to broadband noise.
Unit 284, from Figure \ref{fig:ArnottExamples}A, was used as the
...
Figure \ref{fig:TVNotchresult} shows the notch noise response of the TV cell mode using the best parameters in Table \ref{tab:TVNotchresult}.
...
type, \CTone, is categorised with \CV starting below 0.2 then rising, hence the
name transient, but stays below 0.3.  The second transient type, \CTtwo, is regular in
...
The intracellular voltage statistics, introduced by \citet{PaoliniClareyEtAl:2005}, included the onset ratio,
adaptation shift and offset shift.
The measures of membrane voltage highlight inhibition in \TS cells and were designed to be independent of \RMP. The onset ratio
 measures the change in peak onset depolarisation over the first 5 ms relative to sustained depolarisation after 20 ms.
#+BEGIN_LaTeX
  \begin{equation}
...
The adaptation shift measurement calculates the drop in depolarisation at the end of the stimulus compared to the stable point 20 ms after the onset 
#+BEGIN_LaTeX
  \begin{equation}
  \label{eq:IVstats-Adap} \textrm{Adaptation} = \bar{\mathbf{v}}|_{t=20}^{t=21}\quad - \quad\bar{\mathbf{v}}|_{t=50}^{t=51}. 
\end{equation}
#+END_LaTeX
Greater adaptation to a sustained stimulus is represented by a larger drop in depolarisation. Adaptation is caused by a combination of factors, but is mostly due to reduction excitation from ANF inputs which is caused in turn by adaptation at the IHC-AN synapse. 
...
#+BEGIN_LaTeX
  ...
  \end{equation}
#+END_LaTeX
Negative values indicate hyperpolarisation which is caused by reduction or absence of post-stimulus excitation and/or the presence of inhibition.
...
The \ChTtwo \TS cell model optimisation used unit 01-305-014 (CF 12.4 kHz
\citep*{PaoliniClareyEtAl:2005}) as its exemplar cell. The reference unit had a low depolarising threshold (5
dB \SPL) so the stimulus was set to 35 dB \SPL; a significant difference from the \ChT1
...
The post-tone inhibition of the experimental \ChTtwo unit is the greatest of all the chopper subtypes (-8.82 mV).  
