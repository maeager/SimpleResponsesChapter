...
outside acceptable ranges.  The \RMS error between the CV data of the optimised
...
apparent \RMP) and the strong hyperpolarising dip due to inhibition and recovery
of \HSR activity.  
...
Elevated activity from  GLG, TV and DS cell model inputs at 20 dB re $\theta$ (55 dB
\SPL) reduced the regularity of the spike intervals, and increased CV throughout
the stimulus.  The \RMP for the experimental and model
\ChTtwo units was $-60.63$ and $-64.96$ mV, respectively.  The experimental
\ChTtwo unit had a greater reduction in onset depolarisation (90.2\%) compared to
...
drop (around 6 mV) to that shown in the reference data Figure \ref{fig:CT2results}D, despite
