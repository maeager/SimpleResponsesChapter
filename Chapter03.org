...
in the \GCD by their larger cell body with more cytoplasm  and a surrounding plexus of
dendritic and axonal neurites \citep{Osen:1969,BrawerMorestEtAl:1974,MugnainiOsenEtAl:1980}. The soma diameter of Golgi cells is between 12 to 25 \um in the cat  and from 8 to 18 \um in the rat and the mouse \citep{MugnainiOsenEtAl:1980,FerragamoGoldingEtAl:1998}, while the
...
\citep{Joris:1998,Rhode:1994,RhodeSmith:1986}.  Figure \ref{fig:ArnottExamples} show two \OnC unit rate level responses
to \CF tones and broadband noise \citep{ArnottWallaceEtAl:2004}; the unit 284 from Figure \ref{fig:ArnottExamples}A  was used as the target data to fit the \DS cell model. 

Table \ref{tab:DScellModelSummary}F describes the DS cell model rate-level optimisation routine. Weight and number parameters for the three input connections to \DS cells were modified in the procedure.
...
over \HSR fibres, but the \HSR synapses outnumber the \LSR synapses by a ratio of 2:1.  The best parameters  produced a combined RMS error of 61.89 spikes per second; for non-zero target data elements the mean error relative to the target was 89\%. 
...
\citep{HancockDavisEtAl:1997,HancockVoigt:1999,ReissYoung:2005}.  Consistent with previous \TV cell models, the \CNSM model used the wide-band
inhibition of \DS cells onto \TV cells.  
...
Figure \ref{fig:TVNotchresult} shows the notch noise response best performing parameters (Table \ref{tab:TVNotchresult}) of the notch noise optimisation.
...
The offset of \DS onto \TV cells at 2.1 channels,
equivalent to a mean of 0.14 octaves (0.34 octaves at the lowest channel
and 0.13 at the highest channel), was close to the offset predicted by
\citet{ReissYoung:2005} of 0.3 octaves. 
...
need for \DS cell inhibition with a high number of synapses, \nDSTV.  \LSR fibres have not been proven to preference the deep layers of the \DCN
\citep{Ryugo:2008,MeltzerRyugo:2006,RyugoParks:2003,BabalianJacommeEtAl:2002}.
...
\citep{PaoliniClareyEtAl:2005}: \ChS, \ChTone and \ChTtwo.  The intracellular traces in Figure
...
hand-tuned or guessed based on expected outputs and behaviour. Finally, the background and driving inputs to
...
\citep{WangSachs:1995,WiegrebeMeddis:2004}. Dendritic filtering in each of the cells was not included. 
