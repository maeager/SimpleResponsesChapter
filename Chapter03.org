...
#+END_QUOTE
#+LATEX: {\hfill \textit{\citet[~p.~411]{Shamma:1998}}}
...
Golgi filter model (Section [[Golgi%20Cell%20Model][Golgi Cell Model]]) 
...
neurons in the \CNSM model. Accordingly, an additional delay component was
...
\mathbf{w}_{i\, \small\LSRGLG} (x)= \wLSRGLG/\sqrt(2\,\pi\,\sLSRGLG))\, exp(-((x-i)^2)/(2*\sLSRGLG)),
  \end{equation} 
#+END_LaTeX
\noindent where $x=1,{\rm \ldots},\Nchannels$.
...
#+BEGIN_LaTeX
  ...
  \textrm{g}_i(t) = \sum_{j=1}^{\Nchannels}}{\mathbf{w}_{\LSRGLG}(j) {\rm LSR}_j(t) +
    \mathbf{w}_{\HSRGLG}(j) {\rm HSR}_j(t)}  - \textrm{SR}_{\rm GLG}.
  \end{equation} 
#+END_LaTeX
...
caclulated for each channel in the network (size=\Nchannels})
...
large Type II multipolar neurons in cats and guinea pigs, and DS neurons
...
Section [[Results%20of%20TV%20Cell%20Parameter%20Optimisation%20Using%20Tone%20and%20Noise%20Rate%20Level%20Responses][Results of TV Cell Parameter Optimisation Using Tone and Noise Rate Level Responses]].  The reversal potential of TV cells
...
** Results of TV Cell Parameter Optimisation Using Tone and Noise Rate Level Responses
...
** Optimisation of Wide-band Inhibition onto TV Cells Using Notch Noise Responses
  :PROPERTIES:
  :LABEL: sec:Ch3:DSTV
  :END:
...
interneurons. From the previous optimisation (Section [[Tuberculoventral%20Cell%20Model][Tuberculoventral Cell Model]]),
the preliminary response to noise in \TV cell models was linear between
40 to 80 dB SPL.
