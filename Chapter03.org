...
mostly due to reduction excitation from ANF inputs which is caused in turn by
adaptation at the IHC-AN synapse.
...
Three \CF tone bursts at different \SPL above threshold (20, 30 and 40 dB) were used in
...
\ref{eq:IVstats-Adap}) and average post-tone hyperpolarisation (see offset shift Equation
...
points.  Average \ChS population IV data was limited to one data point at offset
shift at 30 dB re threshold ($1.6\pm1.0$ mV) significant from membrane variations at
rest.  The unfilled triangles in Figure \ref{fig:PaoliniIVdata} were measured
...
for the \ChS subtype of the \TS cell model.  
...
the exemplar unit was 55 dB \SPL, close to the average 47.2 dB \SPL for all \ChTone
units.  The acoustic for the optimisation routine was an 40 dB SPL and the
reversal potential of $-63.9 mV$.
...
was very low (\wGLGTS 1.70742e-05), but the loud input stimulus (70 dB SPL = 30 dB re threshold)
...
call_CT1_rate(SPLreTh=60), call_CT1_rate(SPLreTh=70) and call_CT1_rate(SPLreTh=80)
spikes per second, respectively.  The CV responses of the optimised model were
not sufficient at 30 dB re threshold with the mean over the whole stimulus of
0.308 but the final 10 ms period of the being outside the 0.3 CV threshold for
regularity (CV$_{30-40}=0.42$).
...
# call_CT2_rate(SPLreTh=30)
$161.6$, 
# call_CT2_rate(SPLreTh=40)
$208.8$
and
$267.2$  
# call_CT2_rate(SPLreTh=50)
spikes per second, respectively. 
# call_CT2_fsl(SPLreTh=40) 
The \FSL at 30 dB re threshold ($5.43 \pm 0.13$ ms) showed a precise first spike but the following spikes were too irregular for this unit to be classified a chopper (CV$_{0-10} > 0.3$).
