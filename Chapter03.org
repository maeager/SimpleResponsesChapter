...
#+BEGIN_SRC octave  :export none
 ...
 sprintf("RE tone %.2f%%  noise %.2f%% all %.2f%%  RMS tone %.2f noise %.2f sum %.2f", RE_tone*100, RE_noise*100,RE_all*100, RMS_tone, RMS_noise, RMS)
#+END_SRC

#+RESULTS: DSRL_RMS
: RE tone 53.61%  noise 53.20% all 53.43%  RMS tone 61.61 noise 25.14 sum 48.88
...
Of note in the optimised parameters was the
The case for remaining with the optimised parameters was the success of the
acoustic threshold and for both the tone and noise responses.  The importance of
acoustic threshold accuracy becomes important in later simulations of TV and TS
cells.
...
optimisation routine replicate DS responses to other stimuli, when there are already plenty?}
