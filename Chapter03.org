...
categorised with \CV starting below 0.2 then rising, hence the name transient,
...
and {$21.6\pm11.3$} \um in cats \citep{ReddCahillEtAl:2002}.
The variation is somata sizes based on chopper categorisation has been suggested in guinea pigs
(15.9 \um (\ChS), 17.5 \um (\ChT) \citet{PalmerWallaceEtAl:2003}).
Variation across the frequency regions of the \VCN has also been shown in chinchilla \TS cells; 
cell body sizes in low \CF regions were 20\% larger than cells in high \CF regions
(high \CF {$14.5 \pm 2.0$} \um,  low \CF {$17.6 \pm 2.7$} \um \citep{JosephsonMorest:1998}).
The diameter of the \TS cell model's cell body was 21 \um.
...
*** Physiological properties of TS cells
