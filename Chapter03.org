...
G61.U6, an AVCN \OnC unit (CF 5.69, Th 30 dB SPL), and the effects of the \GABAa
...
#+BEGIN_LaTeX
   ...
   \small\noindent
     ...
     %     \end{center} }
   \vspace{2ex} 
   \yellownote{Update DS Ratelevel result table!!}
   \begin{table}[t!]
     \centering\small
    ...
    \wGLGDS \hfill (nS)  &   [0.01,50]   &  4.83   \\         
    \wHSRDS \hfill (nS)  &   [0.01,50]   & 0.167 \\         
    \wLSRDS \hfill (nS)  &   [0.01,50]   & 10.19  \\         
...
\yellownote{TV RL data has been updated using new DS parameters. Text also needs an update. }
