...
#+BEGIN_SRC octave  :export none
 ...
 RMS= sqrt(meansq((dat(1:end,2) - dat(1:end,3))));
 sprintf("RE tone %.2f%%  noise %.2f%%   RMS tone %.2f noise %.2f total %.2f",RE_tone*100, RE_noise*100,RMS, RMS1, RMS2, RMS1+RMS2)
#+END_SRC

#+RESULTS: DSRL_RMS
: RE tone 28.18%   RMS total 4.48
...
The \DS cell model of the \CNSM model was optimised to have the rate level
and click recovery characteristics of a typical \OnC unit of the \VCN.

The network topology and synapto-physiology of inputs to \DS cells play a
complementary role to the intrinsic characteristics of the neuron.

The
intracellular behaviour of the \DS cell model was implemented using a
single-compartment Type I-II \RM neural model.

The cell's intrinsic behaviour contributes to the rapid response at onset and transient
phasic response.  The phasic response to low current injections is due primarily
to the small number of \KLT channels. which are present in strongly phasic
spherical and globular bushy cells of the \VCN \cite{RothmanManis:2003}.  At
higher current injections, \DS cells respond with regularly spaced \APs
\cite{FujinoOertel:2001,FerragamoGoldingEtAl:1998a,OertelWuEtAl:1990,OertelWuEtAl:1988},
similar to \TS cells.

The intracellular characteristics of \DS cells only plays a s
physiological behaviour of \DS cells to acoustic stimuli


contributes to the ability of \DS

Synaptic inputs and t

The distribution of the \ANF receptive fields contacting \DS cells contribute to
...
centred on CF with spread parameter, \sANFDS = 40 (equivalent to a standard
deviation of 4.47).  The logarithmic topology of the auditory filterbank and
channels in the \CN network mean the 95\% percentile range
octaves.
...
to \TS cells.  The limited activation to noise (Figure
\ref{fig:TVRateLevelResult}B) but strongly activated by \CF tones (Figure
\ref{fig:TVRateLevelResult}A) is characteristic of Type II\space \EIRA units in
the deep \DCN \cite{SpirouDavisEtAl:1999}.  The noise rate level output of the
