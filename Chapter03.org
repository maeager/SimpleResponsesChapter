...
(\sANFDSh and \sANFDSl), and the conduction delay from the \AN (\protect{\dANFDS}).  The
...
\protect{\dANFDS}, was shown in Section \ref{sec:Ch3:Delays}. The first spike latency in high
...
G61.U6, an AVCN \OnC unit (CF 5.69, threshold 30 dB SPL), and the effects of the \GABAa
blocker bicuculine \citep{BackoffPalombiEtAl:1997}.  The normal probe click
response of this unit only reached 100\% of the mask click response until 16 ms
separation. With ionotopopheretic application of bicuculine, the \OnC unit had
full response to the probe click at 1 ms separation. From these results,
...
\yellownote{TODO: Update optimisation results!!! More discussion about
parameters. Ratelevel results reaffirm click recovery optimisation} The \ANF
weight parameters show a clear favouritism toward the \LSR input over \HSR
fibres, but the \HSR synapses outnumber the \LSR synapses by a ratio of 2:1.  At
50 repetitions per stimulus, the best parameters produced an \RMS error of 38.65
spikes per second (individual \RMS error components: tone 44.14 sp/s, noise
30.42 sp/s). For non-zero target data elements the mean error relative to the
target was 0.546.
...
optimised TV cell model (Table \ref{tab:TVRateLevelFig8B}) and the
target data from the unit in Figure 8B in \citet{SpirouDavisEtAl:1999} (see
...
The \TV cell model was able to emulate this response in tones but not optimal
in noise (Figure \ref{fig:TVRateLevelResult}B).

\yellownote{TV RL data has been updated using new DS parameters. Results are much better than before. Text also needs an update and expansion. }
...
#+BEGIN_LaTeX
    ...
    \textbf{Parameters (Units)}& \textbf{Ranges}&\multicolumn{1}{c}{\textbf{Final Value}} \\\otoprule
              \wLSRTV  \hfill (nS)    &  [0.01,50]   & 3.89  \\ 
              \wHSRTV  \hfill (nS)    &  [0.01,50]   & 0.928 \\ 
              \wDSTV  \hfill (nS)      & [0.01,50]   & 1,04 \\ 
                    \nDSTV  \hfill   & [1,500]          & 34 \\
