...
#+LaTeX_CLASS: UoM-draftA-org-article
...
in Section [[sec:Ch2:SynapseModels]].  The synapses act by changing in the
...
\citet{FerragamoGoldingEtAl:1998} confirmed the excitation of Golgi cells from granule cells with \NMDA glutamatergic synapses
...
The \GLG cell model was implemented as an instantaneous-rate Poisson
...
were used in the \GLG cell model.  In the \CNSM model, the connectivity
...
#+BEGIN_LaTeX
  ...
  \caption[Sustained chopper optimisation results]{Optimisation results for the
    ...
    A. PSTH of the \TS cell model (CF=3.872 kHz) to a pure tone at CF at 30 dB re threshold. B. Coefficient of variation Xaveraged over 10 ms periods.  C. Average
    membrane voltage (note shaded areas indicate areas used in the measures in D.
    D. Measures of membrane vol4 hours 3 minutes of battery power remaining (96%)tage including a) onset ratio (Equation~\ref{eq:IVstats-Onset}), b) adaptation of sustained depolarisation (Equation~\ref{eq:IVstats-Adap}), and c) post-tone hyperpolarisation (Equation~\ref{eq:IVstats-Offset}).  The RMP, calculated from -10 to 0 ms, for the
...
\uS, \wHSRTS call_opt_param(param="w.x[hsr][ts]",type="CT2")[:results
replace raw] \uS) were balanced with moderate to low glycinergic
synaptic input (\wDSTS
call_opt_param(param="w.x[ds][ts]",type="CT2")[:results replace raw] \uS,
\wTVTS call_opt_param(param="w.x[tv][ts]",type="CT2")[:results replace
raw] \uS).  The GABAergic input weight was very low (\wGLGTS
