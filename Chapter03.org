...
\yellownote{TODO: Update optimisation results! More discussion about parameters. Ratelevel results reaffirm click recovery optimisation} 
...
2:1.  At 50 repetitions per stimulus, the best parameters produced an \RMS error
of 38.65 spikes per second (individual \RMS error components: tone 44.14 sp/s,
noise 30.42 sp/s). For non-zero target data elements the mean error relative to
the target was 0.546.
...
#+BEGIN_LaTeX
   ...
   \begin{table}[t!]
     ...
     \label{tab:DSRateLevelResults}
         \begin{tabularx}{0.6\textwidth}{X X c} \toprule %    \hdr{2}{}{DS\ Cell Optimisation: Best Fit Parameters } \\ 
...
\yellownote{TODO: state output error value and discuss, then discuss best parameters }
...
#+BEGIN_SRC octave  :export none
 ...
 sprintf("%.1f%%     %.2f (%.2f + %.2f = %.2f)",RE*100,RMS, RMS1, RMS2, RMS1+RMS2)
#+END_SRC

#+RESULTS: DSRL_RMS
: 54.6%     38.65 (30.42 + 44.14 = 74.56)
...
\yellownote{TV RL data has been updated using new DS parameters. Results are
much better than before. Text also needs an update and expansion. }
...
fibres (\wLSRTV \wHSRTS = call_TVopt_param(param="w.x[lsr][tv]",factor="1000")
nS, \wHSRTV \wHSRTS = call_TVopt_param(param="w.x[hsr][tv]",factor="1000") nS).
...
\yellownote{Verification RL plots have not been updated with new TV parameters}
#+BEGIN_LaTeX
  \begin{figure}[t!]
  \centering
  {\figfont{A}\hspace{0.5\textwidth}\figfont{B}\hfill}\\
...
The reduced excitation of TV cell to noise (Figure \ref{fig:TVRateLevelResult}B)
whilst still being activated by \CF tones (Figure \ref{fig:TVRateLevelResult}A)
is characteristic of Type II\space \EIRA units in the deep \DCN.  The noise rate
level output of the optimised \TV cell model was a closer fit to the
experimental unit's response in Figure \ref{fig:SpirouFig8}A rather than the
response used for the optimisation in Figure \ref{fig:SpirouFig8}B.  The strong
level of wide band inhibition from \DS cells in the \VCN limits the role \TV
cells in the \CNSM. \TV cells provide delayed, narrow-band inhibition to \TS cells
...
most important?  Mammalian studies show that the TV cells receive excitatory input from TS
...
call_opt_paramf(param="w.x[lsr][ts]",type="CT1",factor="1000")[:results replace raw] nS,
\wHSRTS call_opt_paramf(param="w.x[hsr][ts]",type="CT1",factor="1000")[:results replace raw]
nS) were balanced with moderate to low glycinergic synaptic input (\wDSTS
call_opt_paramf(param="w.x[ds][ts]",type="CT1",factor="1000")[:results replace raw] nS,
\wTVTS call_opt_paramf(param="w.x[tv][ts]",type="CT1",factor="1000")[:results replace raw]
nS).  The GABAergic input weight was very low (\wGLGTS
call_opt_paramf(param="w.x[glg][ts]",type="CT1",factor="1000")[:results replace raw] nS), but
...
\wHSRTS call_opt_paramf(param="w.x[hsr][ts]",type="CT2",factor="1000")[:results replace raw]
nS) were balanced with moderate to low glycinergic synaptic input (\wDSTS
call_opt_paramf(param="w.x[ds][ts]",type="CT2",factor="1000")[:results replace raw] nS, \wTVTS
call_opt_param(param="w.x[tv][ts]",type="CT2",factor="1000")[:results replace raw] nS).  The
GABAergic input weight was very low (\wGLGTS
call_opt_param(param="w.x[glg][ts]",type="CT2",factor="1000")[:results replace raw] nS), but
the loud input stimulus (70 dB SPL = 30 dB re threshold) would cause reasonable
