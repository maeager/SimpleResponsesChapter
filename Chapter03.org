...
the \CNSM model, discussed in Section [[Neural%20Cell%20Models][Neural Cell Models]], remain
...
# The
# process methods for optimisation were described in Section [[Optimisation%20Techniques][Optimisation Techniques]].
...
#+CAPTION: [Cochlear nucleus stellate microcircuit model]{Cochlear nucleus stellate microcircuit (CNSM) model. The \CNSM model is shown in one iso-frequency plane at best frequency (BF) with incoming ANFs forming the baseline. Dashed lines show connections not in the CNSM model. Each cell type is shown with a characteristic PSTH to short tone bursts and a depiction of their  excitatory-inhibitory response area (EIRA) using sound level (L) versus frequency (F).  Synaptic connections are depicted as: empty triangles - excitatory glutamatergic-AMPA synapse; filled circles - inhibitory glycinergic synapse; and filled rectangles - inhibitory GABA-ergic synapses onto GABA$_{\rm A}$ receptors.  }
#+LABEL: fig:microcircuit
...
\ANFs and were the first model and network parameters to be optimised.
\DS cells were modeled next because they received inputs from \ANFs and
\GABA-ergic Golgi cells. The \TV cells' input network parameters were
optimised next as they were strongly inhibited by \DS cells and received
excitation from \ANFs. Input parameters to \TS cells, which received
inputs from all other cell types in the \CNSM model, were optimised
last.  Due to the variety of possible inputs to \TS cells (and the
...
\citet{RothmanManis:2003b}. The \RM model is discussed in detail in
Section \ref{sec:Meth:cell-models} in Chapter \ref{sec:MethodsChapter}.
...
without the use of function's gradient.  The fitness function converts
the parameters to a single measure by simulating the \CNSM model using
the parameters, then comparing selected neural outputs to experimental
data.  \RMS error is generally used as the comparison measure of similar
data points, for example mean firing rate at different sound levels in a
rate-level simulation.  Mean error relative to the expected or target
values is also used to ensure there is no bias between data points with
large variation in differences.
...
Golgi cell model in the \CNSM model.
...
in the \GCD by their larger cell body with more cytoplasm and a
surrounding plexus of dendritic and axonal neurites
\citep{Osen:1969,BrawerMorestEtAl:1974,MugnainiOsenEtAl:1980}. The soma
diameter of Golgi cells is between 12 to 25 \um in the cat and from 8 to
18 \um in the rat and the mouse
\citep{MugnainiOsenEtAl:1980,FerragamoGoldingEtAl:1998}, while the
...
around 100 \um and the frequency separation laminae in the \VCN core is
...
responses of 6 marginal shell units observed by
...
investigate the temporal behaviour of the \DS cell model.  The second
optimisation used rate level responses to tones and noise from
extracellular recordings of \DS cells \citep{ArnottWallaceEtAl:2004}.

Key elements in the creation of the \DS cell model are shown in the
...
dendrites was based on computational efficiency and the need to ensure
that the model fitted within the criteria for \DS cells.  \DS cells have
electrotonic dendrites and the filtering in \DS cells controls the
...
the experimental design of \citet{BackoffPalombiEtAl:1997}).  Figure
\ref{fig:ClickExample} shows the first two click pairs of the fitness
function stimulus and recording method of the \DS cell model's output.

The six parameters to be fit by the routine were the weights of the \HSR
\ANFs, \LSR \ANFs and \GLG cell model synapses on \DS cells (\wHSRDS,
...
Figure \ref{fig:DSClickRecoveryResult} shows the results of the
optimised parameters, shown in Table \ref{tab:DSClickRecoveryResults},
in the \DS cell model click recovery optimisation routine.  The
optimisation parameters show a clear favouritism toward the \LSR \ANF
input rather than the \HSR input to \DS units.  While this may not seem
ideal for fast coincidence detection, the large number of \HSR synapses
...
show two \OnC unit rate level responses to \CF
tones and broadband noise \citep{ArnottWallaceEtAl:2004}; the unit 284
from Figure \ref{fig:ArnottExamples}A was used as the target data to fit
the \DS cell model.
...
The \ANF weight parameters show a clear favouritism toward the \LSR
input over \HSR fibres, but the \HSR synapses outnumber the \LSR
synapses by a ratio of 2:1.  The best parameters produced a combined RMS
...
spikes/s) have low threshold to sounds and saturate at moderate sound
...
responses at a high rate (approximately 250 spikes/s) were not able to
manipulate the DS cell model in the optimisation other than to set a
threshold for tone and noise sounds.  \LSR fibres (SR=0.1 spikes/s) and
...
in spectral processing may be that of a delayed narrowband inhibitor in
the \VCN, with important echo-suppression characteristics.
\citet{PaoliniClark:1998} suggest that this projection modulates the
response thresholds of \VCN neurons.  \citep{SpirouDavisEtAl:1999} have
suggested that the responses of \DCN principal cells are strongly
...
\citet{ArleKim:1991a} were the first to show Type II \EIRA units with
simple McCullock-Pitts point neuron models of the \DCN.
\citet{BlumReedEtAl:1995} created an abstract steady-state rate model to
determine the effects of network connectivity in the \DCN
\citep{ReedBlum:1995,ReedBlum:1997,BlumReed:1998}. \citet{HancockDavisEtAl:1997}
improved point neuron model by incorporating cross-correlation analysis
in fitting network parameters; further modifications have been included
based on additional physiological observations and modeling
\citep{HancockVoigt:1999,YoungDavis:2002,HancockDavisEtAl:2001,DavisYoung:2000,SpirouDavisEtAl:1999}. The
most recent model from \citet{ReissYoung:2005}, suggests asymmetry in
Type II \DCN units' response to notch noise is due to an offset in
centre distribution of wide-band inhibition from \DS cells; this has not
been confirmed in other studies \citep{LomakinDavis:2008}.
...
\citep{HancockDavisEtAl:1997,HancockVoigt:1999,ReissYoung:2005}.
Consistent with previous \TV cell models, the \CNSM model used the
wide-band inhibition of \DS cells onto \TV cells.
# Studies of the circuitry of the
...
default Type I classic \RM neural model.
...
#+CAPTION: [Experimental data of Type-II~DCN~unit]{Physiological effects of neurotransmitter blockers strychnine and bicuculine on the rate level response of two Type II DCN units to tones and noise \citep{SpirouDavisEtAl:1999}. Data reproduced from Figure 8 in \citet{SpirouDavisEtAl:1999}.}
#+LABEL: fig:SpirouFig8
...
#The Tuberculoventral cell model's optimised parameters were applied to \TV units #across the whole network and tested using tones, noise and tones plus noise stimuli.
...
#+BEGIN_LaTeX
  ...
  %     \includegraphics{ResponsesNoComp/RateLevel/psthsingle50-1.pdf}
  ...
  \begin{figure}[htb]
   ...
   \includegraphics{../../cnstellate/ResponsesNoComp/RateLevel/psthsingle5090-1.pdf}%
    ...
    %  {\figfont{C}\hspace{0.5\textwidth}\figfont{D}\hfill}\\
     ...
     %\includegraphics{}\hfill}\\
      \caption[Optimised TV cell model PSTH responses]{PSTH responses of the optimised TV cell model at the centre of the network (CF=5.8 kHz).
  %   A. Rate level responses to tone, noise and tone plus noise.
  %   B. PSTH at 90 dB SPL\.
  %   C. Response area equivalent using all GLG units in the network.
  %   D. Masked noise-tone response of the central unit to 15 dB masking noise and frequencies one octave above and below its CF.
  ...
  %  \centering
    {\figfont{A}\hspace{0.5\textwidth}\figfont{B}\hfill}\\
    \resizebox{0.95\textwidth}{!}{\hfill%
      \includegraphics[keepaspectratio=true,width=0.48\textwidth]{ResponsesNoComp/RateLevel/response_area-1.pdf}%
      \includegraphics[keepaspectratio=true,width=0.48\textwidth]{ResponsesNoComp/NoiseRateLevel/response_area-1.pdf}%
  %    \includegraphics[keepaspectratio=true,width=0.48\textwidth]{Responses2/RateLevel/response_area-1}\hfill%
  %    \includegraphics[keepaspectratio=true,width=0.48\textwidth]{Responses2/NoiseRateLevel/response_area-1}
  \hfill}%   
...
interneurons. From the previous optimisation (Section \ref{sec:TVRL}), the preliminary response to noise in \TV cell models was
...
Figure \ref{fig:TVNotchresult} shows the notch noise response best
performing parameters (Table \ref{tab:TVNotchresult}) of the notch noise
optimisation.
...
# Run 1B in the se using 70 dB \SPL stimulus.  
# The optimisation produced
# a better result for the offset parameter and the overall error value of
# the fitness function.
The offset of \DS onto \TV cells at 2.1 channels, equivalent to a mean
of 0.14 octaves (0.34 octaves at the lowest channel and 0.13 at the
highest channel), was close to the offset predicted by
\citet{ReissYoung:2005} of 0.3 octaves.  The results in the \TV cell
model notch optimisation show the dominance of \LSR over \HSR fibres in
the number of synapses (29 to 1) and the increased need for \DS cell
inhibition with a high number of synapses, \nDSTV.  \LSR fibres have not
...
#+BEGIN_LaTeX
  ...
  \Eleak $_{\rm TV}$\hfill (mV) &   [-80,-50]   & -65,89 \\ 
        \nDSTV \hfill        &    [0,100]    & 17 \\ 
       \wDSTV  \hfill (nS)      &   [0.01,5]    & 2,9 \\ \midrule
         \multicolumn{2}{l}{RMS error \quad (spikes/s)}        & 20.86 \\ \bottomrule
  \end{tabularx}      
...
of \TS cells, and they confirmed the physiological response of Type I
...
input to determine stochastic excitatory\slash inhibitory inputs was
...
\citep{PaoliniClareyEtAl:2005}: \ChS, \ChTone and \ChTtwo.  The
intracellular traces in Figure \ref{fig:PaoliniAIV} along with the \CV
statistics in Figure \ref{fig:PaoliniCVdata} formed the basis for the
optimisation routine of the \TS cell model of each chopper type in the
\CNSM model.
...
#+ATTR_LATEX: width=0.6\textwidth
#+CAPTION: [Regularity in TS cell subtypes]{Regularity in TS cell subtypes. Average CV inter-spike interval statistic calculated over 10 ms periods. Data reproduced from Figure 2 in \citet{PaoliniClareyEtAl:2005}.}
...
They were not included in the \CNSM model because of the rarity of small
round vesicles on \ChS units \citep{SmithRhode:1989}. Additionally,
\citet{FerragamoGoldingEtAl:1998a} reported that \EPSPs in one \TS cell
were likely to come from only one input. Consequently, because of the
weak influence of \TS cell recurrent connections they were not included
in the \CNSM model.
...
*** Morphology and Cell-based Properties of T Stellate Cells

\TS cells are multipolar neurons of the \VCN and outnumber \DS cells 15
to one in cats \citep{RyugoParks:2003}. The size of their soma is
generally smaller than \DS cells, ranging from 16--20 \um in rats and cats
\citep{PalmerWallaceEtAl:2003,ReddCahillEtAl:2002,DoucetRyugo:1997,DoucetRyugo:2006},
with one study suggesting variation in soma size across the nucleus
(high CF $14.5\pm2.0$, low CF $17.6\pm2.7$, in chinchilla
\citealt*{JosephsonMorest:1998}).  Two to four primary dendrites emanate
from their cell body, extending 150-300 \um parallel to \ANF axons and
do not extend beyond the isofrequency lamina of similar CF units
\citep{SmithRhode:1989,PalmerWallaceEtAl:2003}.  \TS cells are
immuno-labeled as glutamatergic \citep{SmithRhode:1989} and their axons
leave via the trapezoidal body sending excitatory projections to the
inferior colliculus \citep{CantBenson:2003}.

In response to intracellular current injection \TS cells have a Type I
current clamp response. They fire with regularly spaced,
single-exponential undershoot APs and have a linear current-voltage
response
\citep{OertelWuEtAl:1988,OertelWuEtAl:1990,RhodeOertelEtAl:1983,SmithRhode:1989,FengKuwadaEtAl:1994,WuOertel:1984}.
The effective membrane time constant varies between species ranging from
6 to 18 ms \citep{ManisMarx:1991,IsaacsonWalmsley:1995}.  \TS cells have
no \IKLT, but do have two currents active at rest, \IKA and \Ih
\citep{ManisMarx:1991,RothmanManis:2003}.


# \yellownote{Cells Current Clamp Membrane Currents Time Constant (msec) I-V Input R Papers \\ TS Type 1, single exponential undershoot (Feng et al. 1994; Manis and Marx 1991; Wu and Oertel 1984) \\ No Low threshold K (Manis and Marx 1991); IKA has a role in modulating the rate of repetitive firing.  Effect of Inhibition on T stellate cells could be to reset IKA (Rothman and Manis 2003c)}
...
# Linear (Manis and Marx 1991) 447±265 Mohm isolated guinea pig stellate cell type 1 current clamp (Manis and Marx 1991); 44 to 151 MΩ (mean  89.4 ±24.4) mouse slice prep (Ferragamo et al. 1998b); ”stellate” 231±113MΩ, 14.9±9pF primary membrane capacitance, room temp rat (Isaacson and Walmsley 1995) (Bal, Baydas, Naziroglu 2009) Dog 176+/- 35.9 Mohm membrane time constant 8.8 +/-1.4 (n=21)
...
# 2-3 main dend, total dend length $\sim$ 2000\um (guinea pig [\citep{}])
# 75-100\um and 150-300\um parallel, 3--4 primary dendrites (cat 
...
*** Synaptic Inputs and Properties
...
Few synaptic terminals contact the cell body of \TS cells, the majority
of inputs contacting the proximal dendrites \citep{Cant:1981}.



The chopping pattern shown in Figure \ref{fig:microcircuit} is
characteristic of \TS cells. Sustained and transient choppers (ChS and
ChT) are differentiated by the regularity of discharge throughout the
stimulus \citep{YoungRobertEtAl:1988}.


Figure \ref{fig:TSinputs} shows the expected response of a \TS cell to
individual connections from different cells in the \CNSM model.  The
membrane parameters for the single compartment \TS cell model were
default parameters for the Type I-t \RM neural model with the sodium
conductance disabled.  In this example, excitation from the afferent
\ANF inputs (\LSR Figure \ref{fig:TSinputs}A and \HSR Figure
\ref{fig:TSinputs}B) show a large depolarisation.  \HSR inputs show a
rapid onset and a slowly adapting sustained depolarisation.
...
#+BEGIN_LaTeX
  ...
  \begin{figure}[htb]
    ...
    \caption[Intracellular membrane voltage response of a TS cell model to
    isolated synaptic inputs]{Intracellular membrane voltage response of a TS cell
      model to isolated synaptic inputs.  A pure tone stimulus of 8.2 kHz at 85 dB
      SPL was presented to the CNSM model. The CF of the recorded \TS unit was
      8.267 kHz.  Single stimulus responses are shown as a thin line and average
      response over 25 repetitions is shown as the thick line.  A. 30 LSR ANF
      synapses.  B. 20 HSR ANF synapses.  C. 20 DS cell model glycinergic
      synapses.  D. 15 GLG cell model \GABAa synapses.  All weights were set to
      $0.5\,{\rm nS}$ and the sodium conductance (\gNa) was set to zero.  The
      \TS cell model parameters for synapses were: excitatory (tau = 0.36ms),
      glycinergic (tau1=0.4 and tau2=2.5 ms), and GABAergic (tau1=0.26 and
      tau2=5.43 ms).}
    \label{fig:TSinputs}
  \end{figure}
#+END_LaTeX  
...
Figure \ref{fig:TSinputs} omits the non-linear dynamics of the action
potential in the neural model.  The hyperpolarisation activation of \Ih
and \IKA currents due to inhibitory input are not evident.  The second
factor is important in enhancing the first spike response across the T
stellate cells \citep{PaoliniClareyEtAl:2004} to an oncoming stimulus,
since the \OnC units will respond earlier and provide fast inhibition.
...
** Results of Chopper Subtype Optimisation 
...
\citep{PaoliniClareyEtAl:2005}, with \CF of 3.9 kHz and threshold 40 dB \SPL.  This unit had an average membrane 
depolarisation threshold at 10 dB and a firing rate saturation at 75
dB \SPL.   The \TS cell model \ChS unit had a \CF of
3.87 kHz due to the separation of frequency channels in the \CNSM model.
The stimulus for the optimisation routine was a pure tone at 70 dB \SPL, 30
dB above threshold at \CF (3.87 kHz 40 dB \SPL).  A 20 ms delay was included to allow
the model to reach a sustained \RMP.
...
2.868 nS, \wHSRTS = 2.011 nS) and moderate to high inhibition
(\wDSTS 0.291 nS, \wTVTS 0.884 nS, \wGLGTS 0.749 nS).  The low
...
difference between the transient chopper types is reflected in their \CV
values after 10 ms, with the \ChT2 type becoming irregular (i.e. having
a CV value over 0.3) and maintaining that regularity for the remainder
of the stimulus.  \ChT2 has a higher reduction in onset depolarisation
...
The stellate microcircuit in the cochlear nucleus is one of the 
most interesting neural network features of the auditory brainstem.  It enables a
...
Selective processing of two primary modes of ANFs

Attention selective modulation  from higher auditory centres modulating 
...
The function of Golgi cells in the \GCD is very complex and very few
studies have investigated their behaviour /in vivo/ or /in vitro/.  They
interact with many of their granule cell neighbours and with mossy fibre
complexes.  Acoustically driven Golgi cells have a wide dynamic range
which make them well suited to encode the intensity of sound.  Small
cells in the \GCD project to groups of neurons around the superior
olivary complex that are known to be part of the medial olivocochlear
reflex circuit.  The medial olivocochlear circuit regulates the outer
hair cells of the cochlea, the cochlea amplifier.  Golgi
cells are an important link in the medial olivocochlear circuit and
their accurate representation of sound levels is important in the \CNSM model.

The Golgi cell model reproduces the monotonic rate-level response
observed in experimental data of a single unit in the marginal shell of
the \VCN \citep{GhoshalKim:1997}.  The filter model and spiking process
are an effective and fast means of generating the expected Golgi cell
behaviour.  The strength of ANF inputs was dominated by \LSR fibres, as
expected by their wider dynamic ranges; however \HSR fibres were needed
to begin the GLG cell model's rate-level rise at earlier sound levels.
...
The output characteristics of DS cells after onset are determined by a
combination of intrinsic and extrinsic properties. The current clamp
response of \DS cells is bimodal; firing only at onset for low
excitatory current and firing repetitively for higher input
currents. The Type I-II \RM neural model contains small traces of \IKLT,
which in larger traces is responsible for the phasic (Type II) responses
characteristic of \AVCN bushy cells, reproducing the bimodal current
clamp behaviour in DS cells.  Extrinsic properties influencing the \OnC
behaviour post onset include a reduced level of excitation and delayed
inhibition.  The reduced level of excitation post-onset is observed in
\ANFs with a fast adaptation in the first 10 ms and slow adaptation
throughout the stimulus.  Inhibition is present in DS cells with glycine
and GABAergic synapses
\citep{SmithRhode:1989,FerragamoGoldingEtAl:1998a}. In the \CNSM model,
the \DS cell only receives inhibition from GABAergic Golgi cells. This
connection represents the actual Golgi cell connections to \DS cells at
the distal end of their dendrites and somatic GABAergic inputs (most
likely from peri-olivary neurons).
...
The primary purpose of this optimisation study was to pinpoint the
...
hand-tuned or guessed based on expected outputs and behaviour. Finally,
the background and driving inputs to the network set the environmental
conditions for genuine realistic models to be compared with experimental
animal network under the same conditions and inputs.
...
\citep{WangSachs:1995,WiegrebeMeddis:2004}. Dendritic filtering in each
of the cells was not included.
