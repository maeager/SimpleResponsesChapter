...
#+BEGIN_LaTeX
  ...
  \begin{tabularx}{0.9\textwidth}{p{1.5in}XXXXX}
  \toprule
...
The \CNSM model shown in Figure \ref{fig:microcircuit}, shows the TS cell model receives synaptic inputs from all other neurons in the microcircuit.
...
cells cell body and are mainly on proximal dendrites cats and rodents
...
the axon hillock of \TS cells in chincillas. This property would remove the
...
\TS cells is small relative to its multipolar cousin, \DS cells.  An /in vivo/
study in mice suggested \TS cells receive as little as 5 excitatory inputs
\citep{FerragamoGoldingEtAl:1998a}.
...
used in all three \TS cell model optimisation.  The threshold was assumed to be
...
shift at 30 dB re threshold ($1.6\pm1.0$ mV) significant from membrane variations
at rest.  The unfilled triangles in Figure \ref{fig:PaoliniIVdata} were measured
...
very high excitatory input (\wLSRTS = call_opt_param(param="w.x[lsr][ts]",type="CS")[:results replace raw] S, 
\wHSRTS =  call_opt_param(param="w.x[lsr][ts]",type="CS")[:results replace raw] S)
and moderate to high inhibition (
\wDSTS = call_opt_paramf(param="w.x[ds][ts]",type="CS")[:results replace raw] S, 
\wTVTS = call_opt_paramf(param="w.x[tv][ts]",type="CS")[:results replace raw] S,
...
from the initial value (4.907e-04 S), causing a decrease in the input
...
\ChTone units.  The acoustic for the optimisation routine was an 40 dB SPL and
the reversal potential of $-63.9 mV$.

Excitatory synaptic weights (\wLSRTS call_opt_param(param="w.x[lsr][ts]",type="CT1")[:results replace raw] S, \wHSRTS call_opt_param(param="w.x[hsr][ts]",type="CT1")[:results replace raw] S) were
balanced with moderate to low glycinergic synaptic input (\wDSTS
call_opt_paramf(param="w.x[ds][ts]",type="CT1")[:results replace raw] S,
\wTVTS 
call_opt_paramf(param="w.x[tv][ts]",type="CT1")[:results replace raw] S).  The GABAergic input weight was very low (\wGLGTS
call_opt_paramf(param="w.x[glg][ts]",type="CT1")[:results replace raw] S), but the loud input stimulus (70 dB SPL = 30 dB re threshold) would
...
Excitatory synaptic weights (\wLSRTS call_opt_param(param="w.x[lsr][ts]",type="CT2")[:results replace raw] S, \wHSRTS call_opt_param(param="w.x[hsr][ts]",type="CT2")[:results replace raw] S) were
balanced with moderate to low glycinergic synaptic input (\wDSTS
call_opt_param(param="w.x[ds][ts]",type="CT2")[:results replace raw] S,
\wTVTS 
call_opt_param(param="w.x[tv][ts]",type="CT2")[:results replace raw] S).  The GABAergic input weight was very low (\wGLGTS
call_opt_param(param="w.x[glg][ts]",type="CT2")[:results replace raw] S), but the loud input stimulus (70 dB SPL = 30 dB re threshold) would
cause reasonable inhibition to the \TS cell throughout the stimulus.

The final error of the optimised \ChTtwo unit was call_opt_error(type="CT2")[:results replace raw].
...
be classified a chopper (CV$_{0-10} > 0.3$).
