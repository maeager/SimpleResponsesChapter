...
real neural networks.  Choices, guess and assumptions continue to be
...
needs to be developed in sequential stages.  \yellownote{what parameters
are we talking about here? introduce Fig} The parameters in the
sequential optimisation stages should be chosen so that they do not
influence the properties of previously fitted parameters. In this
chapter, the connections within the \CNSM model (Figure
\ref{fig:microcircuit}) were developed using such a sequential method of
optimisation steps. The parameters of each cell type were determined
based on experimental data or were optimised based solely on responses to
simple stimuli. Each cell type was typified with its characteristic
\PSTH in response to short tone bursts
...
\yellownote{TODO: This may need to go in Methods chapter}
...
Bruce model \citep{BruceSachsEtAl:2003,ZilanyBruce:2006,ZilanyBruce:2007}, and the Zilany model \citep{ZilanyBruceEtAl:2009}.
...
In order to specify how \ANF and 
\GABA-ergic inputs regulate the click
recovery response in \DS cells, a simple parameter optimisation was
performed.  The \DS neural model was implemented with a
...
intra-nuclear co-localised GABAergic input from Golgi cells.  
The \DS cell model's input parameters were pre-emptively fixed.  These included the number of \GLG to
\DS synapses ($\nGLGDS = 25$), the spread of \ANFs to \DS cells
(\sANFDSh and \sANFDSl), and the conduction delay from the auditory
nerve (\dANFDS).  
The \ANF spread onto \DS cells is well documented
...
The additional delay parameter, \dANFDS, was shown in Section \ref{sec:Ch3:Delays}.
 The first spike latency in high \CF \DS cells ($2.8 \pm 0.09$ ms) is
precise and faster than other stellate neurons in the \VCN
\citep{RhodeSmith:1986}.  The additional of 1.2 ms to \ANF to \DS input
connections is a combination of axonal conductance and dendritic delay.  
...
Figure \ref{fig:DSRateLevelResult} shows the final results of 
the \DS model rate level optimisation.  The
optimisation parameters show a clear favouritism toward the \LSR input
rather than the \HSR input to \DS units.  While this may not seem ideal
for fast coincidence detection, the large number of \HSR synapses makes
up for the small weight that was obtained in the optimisation.

#+CAPTION: [Rate level optimisation results in the DS cell model]{Tone and noise rate level behaviour in the DS cell model (CF 6.3 kHz) was optimised to experimental data in \citet{ArnottWallaceEtAl:2004}. The target response (blue circles) is obtained from Fig. 3 in \citet{BackoffPalombiEtAl:1997}, }
#+LABEL: fig:DSClickRecoveryResult
...
\TV cells are glycinergic, inhibitory cells found in the
deep layers of the \DCN that send axon collaterals to the \VCN.  They
are characterized as having unique cell bodies and synaptic characteristics that distinguish them from other cells in the deep layer, Giant cells and Golgi-stellate cells \citep{SaintBensonEtAl:1991,WickesbergOertel:1993,Alibardi:2000b,Alibardi:1999}.
  Anterograde labeling in the \DCN
...
\citep{OertelWu:1989,ZhangOertel:1993b}.  Ultra-structural labeling of
...
\citep{EvansNelson:1973,YoungBrownell:1976}.  Type I \EIRA units reflect the responses of single \ANFs 
\TV cells have a
non-monotonic response to tones with increasing sound level and respond
poorly to broadband noise, categorised as a Type-II \EIRA unit
...
Recordings /in vitro/ show that tuberculoventral cells respond with
 regular "chopper"-like behaviour to voltage-clamped current inputs
...
response, characteristic of the type-Ic neural model(Figure
...
70 dB \SPL stimulus.  \yellownote{TODO: Explain the table below more. }
