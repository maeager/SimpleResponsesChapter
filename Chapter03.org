...
#+LATEX_HEADER:\graphicspath{{../SimpleResponsesChapter/gfx/}{../../cnstellate/Responses_CT1}{../../cnstellate/Responses_CS}{../../cnstellate/MASSIVE/cnstellate/Responses_CT2}}
...
#+LABEL: fig:microcircuit
[[../SimpleResponsesChapter/gfx/CNcircuit-detailed2.pdf]]
...
cross dependencies within the isofrequency microcircuit and across the whole
...
Golgi-stellate and cartwheel cells, with similar Type I\slash III \EIRA properties.
...
synaptic connections in the \CNSM model involving \ANFs. \FSL is calculated from the
first stimulus-induced spike of a neuron that disregards spontaneous spikes.
Mean \FSL to short \CF tones and click stimuli was used to set delay times
between \ANFs and \CN cells
...
delay. The \FSL measured in high \CF units in the ARLO \AN model
...
#+BEGIN_LaTeX
  {
    ...
    \begin{table}[tp]
      ...
      \begin{tabularx}{0.95\textwidth}{p{1.9in}XXXXX} \toprule
...
similar data points, e.g. mean firing rate at different sound levels in a
...
#+BEGIN_LaTeX
\begin{equation} \label{eq:Ch3:MAR}
...
Figure \ref{fig:ANFVerificationRL} shows the output of the \HSR and \LSR \ANFs
in the \CNSM model to the verification stimuli 
\citep[modified~][ AN model]{ZilanyCarney:2010}. The V-shaped tuning of the \HSR \ANFs to pure tone
bursts (Figure \ref{fig:ANFVerificationRL}A) has no inhibitory areas and is
classified in \EIRA as Type I. The equivalent \Qten, bandwidth measured in
individual units at 10 dB above threshold, can be measured across different
units. The \HSR units were active over 4 channels (CF range of 880 Hz) around
the central unit (CF 4.513 kHz) at 20 dB \SPL (threshold at 10 dB \SPL) and the
equivalent \Qten was 0.195 octaves. The noise response area of the \HSR \ANFs is
shown in Figure \ref{fig:ANFVerificationRL}B. The \HSR unit's on-CF rate level
response in Figure \ref{fig:ANFVerificationRL}C has a \SR=45 sp/s, a \DR between
25 and 30 dB and a maximum rate of 233.2 sp/s at 65 dB SPL. The maximum rate of
the \LSR \ANF was 77.1 sp/s and an acoustic threshold at 25 dB \SPL. The noise
rate level of the central unit (Figure \ref{fig:ANFVerificationRL}D) had a
higher threshold at 25 dB \SPL and two stages of dynamic range firing rate
increase before a maximum rate of 243.5 sp/s at 90 dB \SPL. The \LSR unit's
noise rate level was monotonic with a maximum rate of 76.6 sp/s at 110 dB \SPL.
#+BEGIN_LaTeX
  {%
    \begin{figure}[tp!]
      \centering
...
#+ATTR_LATEX: width=\textwidth,keepaspectratio
...
over 10 msec, with a mean of 3.75 msec. The acoustic latency closely matches
the minimum latency of \EPSPs to \AN shocks recorded in mice /in vitro/
preparations \citep[1.3~msec,][]{FerragamoGoldingEtAl:1998}. \EPSPs with longer
latencies may be due to Type II \ANFs, which have an estimated theoretical
latency of 10 msec \citep{Brown:1993}, or from polysynaptic excitation by
granule cells.
...
cell model had a fitness score of 4.48 spikes per second using the \RMS error. A
normalised metric that takes into account the different firing rate magnitudes
at each sound level relative to the target response showed a \MAR error of
21.5\%.  The discrepancies at low \SPL contributed the most to the \MAR error
despite a reasonably good fit by observation.
...
#+BEGIN_LaTeX
  \begin{figure}[t!]
    ...
    \caption[Optimised GLG cell model responses to tones and noise]{Response of
      optimised GLG cell model at the centre of the network (CF 5.8 kHz). A. Four
      simulated PSTHs of the \GLG cell model to tones at 50 and 90 dB SPL and
      broadband noise (BBN) at 60 and 100 dB SPL (50 msec duration, 20 msec
      delay). B. Four experimental PSTHs from marginal shell units
      \citep[from~Figure~12,][]{GhoshalKim:1996a}.}
    \label{fig:GolgiVerificationPSTH}
  \end{figure}
#+END_LaTeX


#+ATTR_LATEX: width=0.7\textwidth,keepaspectratio
...
Figure \ref{fig:GolgiVerificationRL}D was monotonic. The average gradient of
...
of 2 channels (i.e. SD= $\sqrt{2}$) with zero offset, which corresponds to
...
\wLSRDS, \wGLGDS); the \GABAa synapse for fast and slow decay constants
(\tGABAone, \tGABAtwo); and the \DS cell maximum leak conductance (\gleak).
...
optimisation routine used the \MAR error (between the reference and model
response) divided by the reference \DS cell's response. The combined relative
mean relative error (optimised \DS cell model relative to the reference \DS cell
excluding non-zero elements) in Figure \ref{fig:DSRateLevelResult} was
58.59\%. The poor fit of the tone rate level response (73.36\% \MAR error)at
higher sound levels is compensated for by the good fit to the experimental
target response at low levels, especially around the acoustic threshold. The
noise rate level response had a lower \RMS error (26.1 sp/s) and lower \MAR
error (37.25\%) across the sound levels; however the \DS cell model was active
to noise at 40 dB \SPL (well below the threshold of the target \DS unit, around
60 dB SPL).
...
model reduced from 24.3 msec at 50 dB \SPL to 23.40 msec at 90 dB \SPL. The
onset \PSTH responses to frozen broadband noise did not have the second chopping
spike but did have other peaks determined by the spectral peaks in the unit's
receptive field.  (Figure \ref{fig:DSVerificationPSTH}C and D). For comparison,
on and off-\CF\space \PSTH tones responses in three rat \DS cells are shown in Figure
...
The discrepancy in the rate level optimisation show a limitation in fitting a
\DS cell model to exemplar cell data and has implications for the design of the
...
limitations in the ANF spontaneous rates has direct impact on the distribution
of acoustic thresholds (CF tone HSR: approx.\space 0 dB \SPL, \LSR: approx.\space 30 dB
\SPL), which varies greatly in \HSR units in cats and other mammals
\citep{Schmiedt:1989,HeilNeubauerEtAl:2007,Liberman:1993,Liberman:1978,WangPowersEtAl:1997}.
...
acoustic threshold for \CF tones and noise; however, the \DS cell model's
responses at high sound levels diverged from the experimental unit's response.
The errors in the \DS cell model's rate level optimisation are a consequence of
...
further development of the model is not necessary.

\glsunset{IPSP}

The click recovery optimisation was used to modify the physiological dynamics of
the \GLG cells' \GABAa synapses with \DS cells, particularly the \GABAa decay
...
Two important results came out of the click recovery optimisation. First, the
\GABAa synapse onto \DS cells needs to have a fast onset. The initial response
of the \DS cell model to click pairs separated by 2 msec required a probe
...
optimisation found a decay constant of 5.6 msec achieved the best results. In
\MNTB neurons of mature rats, the fast decay time constant of \GABAa \IPSPs had
a mean of 7 msec and standard deviation of 2 msec
\citep{AwatramaniTurecekEtAl:2005}. Slower components of the \GABAa synapse
with slower decays and synapse depletion were ignored in the \CNSM model but may
be important in more complex stimuli.
...
of \LSR and \HSR \ANF inputs (\HSR 0.167 nS, \LSR 11.03 nS).
...
physiological characteristics of \DCN neurons are categorised into \EIRA types,
first in cats \citep{EvansNelson:1973} then refined
\citep{YoungBrownell:1976,YoungVoigt:1981,YoungVoigt:1982,ShofnerYoung:1985,SpirouDavisEtAl:1999}.
Type I \EIRA units reflect the excitatory response area of single \ANFs. \TV
cells have a non-monotonic response to tones with increasing sound level,
respond poorly to broadband noise, and are categorised as Type II \EIRA units
\citep{SpirouDavisEtAl:1999,NelkenYoung:1997,ReissYoung:2005,DavisVoigt:1997,VoigtYoung:1980}. Units
with Type II \EIRA responses are sharply tuned and have thresholds 10 dB higher
than other units with which they are intermingled and do not respond to
broad-band noise
...
#+CAPTION: [Typical responses of a Type II DCN unit]{Response area and rate-level responses to tones and noise of a single Type II DCN unit. Reproduced from Figure 1 in \citet{SpirouDavisEtAl:1999}.}
...
cell model was implemented with a single-compartment conductance Type I-c \RM
neural model, a classic regular firing unit \citep{RothmanManis:2003b}. The
reversal potential of \TV cells (\Erev) was set to a lower \RMP (-70 mV) to
increase the \AP threshold. The synaptic inputs to the \TV cell model included
excitation from \HSR and \LSR\space \ANFs, and inhibition from \DS cells and
\GLG cells. \GLG cell inputs represent characteristic GABAergic inhibition of
\TV cells from local neurons (Golgi-stellate and cartwheel cells) that are not
included in the \CNSM model. These units have a similar Type I\slash III \EIRA
characteristic to \GLG cells. The weights (\wLSRTV, \wHSRTV) of \ANF synapses
were used in the optimisation procedures to allow for fine adjustments to the
strength of excitation. The numbers of synapses (\nLSRTV=20, \nHSRTV=20,
\nDSTV=30, \nGLGTV=10) were fixed at appropriate values. The \TV cell model
received \ANF inputs on-\CF from only one frequency channel at the same \CF
(\sANFTV=0). The strength of inhibition in the \TV cell model was controlled by
the weight of \DS and \GLG cell inputs (\wDSTV, \wGLGTV). The parameters
controlling the distribution of \DS cell inputs to \TV cells were fixed
($\sDSTV=30$, $\oDSTV=0$) in the rate level optimisation. The offset parameter,
\oDSTV, is the focus of the notch optimisation in Section \ref{sec:Ch3:DSTV}.
...
Figure \ref{fig:SpirouFig8} shows the rate level output behaviour of two \TV
cells in control environments and environments that block important
neurotransmitters \citep{SpirouDavisEtAl:1999}. Figure \ref{fig:SpirouFig8}A
shows a Type I\slash III \EIRA unit in the deep layer of the \DCN. The suppressed
noise response in Figure \ref{fig:SpirouFig8}B categorises this neuron as a
typical Type II \EIRA unit. The \TV cell in Figure \ref{fig:SpirouFig8}B was used
as the reference in the \TV cell model optimisation. The reference \TV unit was
recorded with ionotopopheretic application of strychnine and bicuculine, which
directly affect synaptic connections relevant to the \CNSM model,
e.g. glycinergic \DS cell connections and \GABAa-ergic \GLG cell connections. For
the \TV cell model's rate level optimisation, three \TV cell models were
simulated. The first \TV cell model was simulated with all connections active and
was compared against the control responses. The second \TV cell model was
simulated without \DS cell connections and compared against the strychnine
responses. The final \TV cell model was simulated without \DS or \GLG connections
and compared against bicuculine responses. The sound level units (dB
Attenuation) were drawn from an acoustic calibration technique close to the ear
drum of the animal \citep{SpirouYoung:1991,RiceYoungEtAl:1995}, where 0 dB
attenuation is approximately 100 dB \SPL.
...
from the unit in Figure 8A in \citet{SpirouDavisEtAl:1999} (see Figure
\ref{fig:SpirouFig8}). Figures \ref{fig:TVRateLevelResult}A and B show the tone
and noise rate level response of each of the inputs to the \TV cell model.
...
The optimised \TV cell model response in Figure \ref{fig:TVRateLevelResult}C
shows a poor fit for the control tone rate-level output, especially at high
SPL. The \RMS of the tone rate level response in the experimental control and
optimised \TV cell model was call_TV_RMS_error(errstr="# RL Error ") sp/s. This
response was non-linear and showed strong inhibitory effects without recovery to
a stable firing rate. The discrepancy is problematic for this particular
experimental data, but the \TV cell model's response is similar to other Type II
\DCN units in Figures \ref{fig:SpirouFig1}B and C which also showed strong
suppression of \CF tones at high level. The acoustic threshold in the \HSR on-\CF
unit was 10 dB less than the experimental \TV unit and was almost saturated at 20
dB SPL where the target \TV cell was just beginning to fire.
The noise rate level \RMS error was significantly
better at
call_TV_RMS_error(errstr="# NRL Error ") sp/s, an important factor in the
Type II \EIRA response of the \TV cell model. For the strychnine-affected
experimental data and response of the \TV cell model without \DS cell inputs
(Figures \ref{fig:TVRateLevelResult}E and F), the \RMS error for the tone and
noise rate level was
call_TV_RMS_error(errstr="# RL without DS Error ") and
call_TV_RMS_error(errstr="# NRL without DS Error ") sp/s, respectively. The
model's response was well fitted, with only a limited amount of error in
mid-level tones and a shift in threshold by 15 dB in noise stimuli.
For the bicuculine-affected experimental data and equivalent \TV cell model
(Figures \ref{fig:TVRateLevelResult}G and H), the \RMS errors were
call_TV_RMS_error(errstr="# RL wo GLG Error ") and
call_TV_RMS_error(errstr="# NRL wo GLG Error ") sp/s, for tone and noise rate
level, respectively. The \DS cell
inhibition present in the bicuculine-affected scenario reduced the maximal
output and saturation rate of the model's rate level response. The timing and
coincidence of onset inhibition dramatically affected the \TV cell model where
the activity in the model was predominantly in the second half of the 50 msec
stimulus. In response to noise, the \DS cell model's rate level was slightly
higher but the \LSR \ANF noise rate level flattened out (Figure
\ref{fig:TVRateLevelResult}B) contributing to a low saturation rate in the \TV
cell model (Figure \ref{fig:TVRateLevelResult}H). The final error used in the
optimisation routine was call_TV_RMS_error(errstr="# Final error ") sp/s, a
summation of the six rate-level \RMS errors.

Misalignment of the rate-level curve in the input on-CF HSR units had
detrimental affect on the TV cell optimisation. The acoustic thresholds of the
HSR units to pure tones were 10 dB lower than the target \TV unit (Figures
\ref{fig:TVRateLevelResult}A and C for tones). The acoustic threshold of the \TV
cell exposed to strychnine (Figure \ref{fig:TVRateLevelResult}F) was 15 dB
greater than the threshold of the \HSR unit (Figure
\ref{fig:TVRateLevelResult}B). The control tone and noise comparisons were
weighted five times more than the strychnine and bicuculine comparisons in the
optimisation and the \wHSRTV parameter would be less favourable due to the
misalignment of its rate-level response.
...
#+BEGIN_LaTeX
  ...
  \textbf{Parameters (Units)}& \textbf{Ranges}& \multicolumn{1}{c}{\textbf{Final Value}} \\\otoprule
     \wLSRTV  \hfill (nS)    &   [0.01,100]    & 5.17  \\
     \wHSRTV  \hfill (nS)    &   [0.01,100]    & 0.355 \\
     \wDSTV  \hfill (nS)     &   [0.01,100]    & 1.97 \\
     \wGLGTV \hfill (nS)     &   [0.01,100]    & 1.42 \\
...
fibres (\wLSRTV =
call_TVopt_param(param="w.x[lsr][tv]",factor="1000")[:results replace raw] nS, \wHSRTV =
call_TVopt_param(param="w.x[hsr][tv]",factor="1000")[:results replace raw] nS).
Each of the input cell types to the \TV cell model had 30 synaptic inputs. All
\ANF inputs were on-\CF and were chosen from different \HSR and \LSR fibres in
the same frequency channel.
The optimum inhibitory weight from \DS cells was
call_TVopt_param(param="w.x[ds][tv]",factor="1000")[:results replace raw] nS.
The \DS cell inputs were selected from frequency
channels on and around the \CF of the \TV cell (Gaussian distribution, spread
parameter \sDSTV = 30).
Due to the \OnC response properties of \DS cells, the inhibition was most
effective at stimulus onset and at higher sound levels.
The weight of \GLG cell
inputs was comparable to \DS cells' synaptic weight (\wGLGTV =
call_TVopt_param(param="w.x[glg][tv]",factor="1000")[:results replace raw] nS) as
well as the same number of connections. The \GLG cell model had a low threshold
and a steady increase in firing rate with increasing level (Figure
\ref{fig:TVRateLevelResult}A and B), along with its longer time constant,
suggesting a continuous monotonic \GABAa inhibition regulating the activity in the
\TV cell model.


# \citep{ReissYoung:2005,NeuertVerheyEtAl:2004,Rhode:1999}


#  input was 47.65 sp/s, which had a similar threshold to
...
#+name: TVopt_error
#+BEGIN_SRC sh :results replace raw
 grep -e '^# .*rror' ../SimpleResponsesChapter/gfx/TV_RateLevel/TV_RateLevel.Fit.dat | awk -F'=' '{print $2}'
 ## '{$5=sprintf("%g",$5);printf("%.1f",$5)}'
#+END_SRC

#+name: TV_RMS_error
#+BEGIN_SRC sh :results replace raw :var errstr="# RL Error "
 grep -e "${errstr}" ../SimpleResponsesChapter/gfx/TV_RateLevel/TV_RateLevel.Fit.dat | awk -F'=' '{$2=sprintf("%g",$2);printf("%.1f",$2)}'
#+END_SRC
...
#+BEGIN_SRC octave  :export none
 ...
 RE_tone_control= mean(abs(dat(16:end,2) - dat(16:end,4))'./max(dat(16:end,[2 4])'));
 RE_tone_nods= mean(abs(dat(16:end,3) - dat(16:end,5))'./max(dat(16:end,[3 5])'));
 RE_tone_noglg= mean(abs(dat(16:end,10) - dat(16:end,11))'./max(dat(16:end,[10 11])'));
 RE_noise_control= mean(abs(dat(1:15,2) - dat(1:15,4))'./max(dat(1:15,[2 4])'));
 RE_noise_nods= mean(abs(dat(1:15,3) - dat(1:15,5))'./max(dat(1:15,[3 5])'));
 RE_noise_noglg= mean(abs(dat(1:15,10) - dat(1:15,11))'./max(dat(1:15,[10 11])'));
 ...
 sprintf("%.1f%% %.1f%% %.1f%%     %.2f (%.2f + %.2f +%.2f + %.2f +%.2f + %.2f)", \
  RE_control*100,RE_nods*100,RE_noglg*100, \
  RMS_tone_control+ RMS_tone_nods+RMS_noise_control+RMS_noise_nods+RMS_tone_noglg+RMS_noise_noglg, \
  RMS_tone_control, RMS_tone_nods,RMS_tone_noglg, RMS_noise_control,\
  RMS_noise_nods, RMS_noise_noglg)
#+END_SRC
75.6% 33.1% 71.7%     644.89 (142.83 + 92.94 +181.24 + 65.53 +65.53 + 96.81)
...
The notch sweep sets used by \citet{ReissYoung:2005} were generated with
...
increase the \AP threshold for excitatory inputs to generate action
potentials. To allow for this, the reversal potential of the \TV cell model
(\Erev) was used as a parameter in the optimisation routine with a range from
-80 to -50 mV.
...
# Rate-level response to tones and noise was used to verify the optimisation of
# the network parameters influencing \TV cells.
...
#+BEGIN_LaTeX
  ...
  %  %  {\figfont{A}\hspace{0.5\textwidth}\figfont{B}\hfill}\\
  {\hfill\resizebox{0.8\textwidth}{!}{%
  \includegraphics{../SimpleResponsesChapter/gfx/Responses/psthblock-1.pdf}}%
  \hfill}\\
  %  %  {\figfont{C}\hspace{0.5\textwidth}\figfont{F}\hfill}\\
       \caption[Optimised TV cell model PSTH responses]{PSTH response of optimised TV cell model at the centre of the network (CF 5.8\ kHz) to CF tones and noise (50 msec duration, 50 repetitions).
      PSTH responses to CF tones at 40 (A) and 70\ dB SPL (B), and broad-band noise (BBN) at 40 (C) and 90\ dB\ SPL (D).
...
#+BEGIN_LaTeX
  ...
  \label{fig:TVVerificationRL}
  \end{figure}
#+END_LaTeX

The TV cell model, with optimised parameters from both the rate level and notch
optimisations, was tested using tones and noise stimuli across the whole \CNSM
network. Figure \ref{fig:TVVerificationPSTH}A shows the PSTH response of the
central \TV unit (\CF 4.5 kHz) at 40 dB \SPL (classified as \OnL). The
first spikes occur at 4.3 msec after the stimulus onset (similar to the
non-optimised \TV cell model's mean \FSL in Section \ref{sec:Ch3:Delays}). The
characteristic chopping response of the Type I-c \RM neural model
\citep{RothmanManis:2003b} is restricted from repetitive firing due to \GABAa and
glycinergic inhibition, particularly at the onset of excitation. With increasing
sound intensities the PSTH become highly irregular and suppressed (Figure
\ref{fig:TVVerificationPSTH}B). In response to noise, the \PSTH of the \TV cell model had
minimal activity at 40 and 90 dB SPL (Figure \ref{fig:TVVerificationPSTH}C and
D) typical of the Type II \EIRA units in the deep layer of the \DCN
\citep{SpirouDavisEtAl:1999,Rhode:1999,DavisYoung:2000}.


Figure \ref{fig:TVVerificationRL} shows the response of all the \TV cells to the
full battery of stimuli. The rate response to tones (Figure
\ref{fig:TVVerificationRL}A) is limited to a narrow field around the central
channel (\CF 4.513 kHz). The bandwidth of active units 10 dB above threshold was
only three channels with a \Qten of 0.143 octaves , measured at 20 dB \SPL. Due
to the strong \DS cell inhibition, the responses above 60 dB SPL were suppressed
for all \TV cells. The on-\CF tone rate level response in Figure
\ref{fig:TVVerificationRL}C shows a rapid rise in excitation at 10 dB SPL
followed by a gradual rate reduction at 40 dB \SPL and then a sudden drop in rate
from 65 dB \SPL. The TV cell model's response to noise was suppressed for all
units (Figure \ref{fig:TVVerificationRL}B). Activity above small spontaneous
rates was present only at low sound levels that follow the acoustic threshold
curve in the ANF inputs. The central unit's maximum rate was 38 sp/s at 40 dB
\SPL (Figure \ref{fig:TVVerificationRL}D) was less than half the maximum rate for
all \TV cell's to noise (98 sp/s in unit 93, CF 30.8 kHz, 40 dB \SPL).
...
#+BEGIN_LaTeX
  ...
  \centering
    ...
    \caption[Response of optimised TV cell to notch noise and band-pass noise sweeps]{Response of optimised TV cell (CF 12.76 kHz) to notch and band-pass
...
The discrepancies in the \TV cell's rate level optimisation reflect the
limitations of the CNSM model and the optimisation process. The acoustic threshold
and rate-level response gradient in each of the input cell types play a
critical role in the output responses of \TV cells. The cytoarchitecture of \TV cells
and their synaptic inputs may be more elaborate than originally proposed
\citep{Rubio:2005,Rubio:2006,Alibardi:2006,MunirathinamOstapoffEtAl:2004}. The
\TV cell also receives more complex EIRA inputs in the deep layer of the \DCN than are
accounted for in the model.
# The characteristics of the DCN circuit are broadly similar to
# the cerebellum, with TV cells aiding the detection of spectral notch cues at
# high frequencies in DCN principal cells \citep{DavisYoung:2000,YoungDavis:2002}.
\GLG cell inputs to the TV cell model were included to replace \GABA and mixed
\GABA-glycinergic neurons. This approach, while limited, was satisfactory for
demonstrating the performance of the \TV cell model with all the features of a
Type II \EIRA\space \DCN unit: no spontaneous activity, non-linear rate level
response to tones and no response to noise.
...
# \note{TODO: Discuss the \TV cell Notch optimisation. Is the goal of
...
categorised with \CV starting below 0.2 then rising but staying below 0.3. The
second transient type, \ChTtwo, is highly regular in the first 10 msec period
(CV less than 0.2), but rises above 0.3 throughout the rest of the
stimulus. \OnC units can also be misclassified as \ChTtwo due to two to three
regular onset spikes followed by irregularly spaced spikes.

#+BEGIN_LaTeX
  \begin{figure}[thp]
    \centering
  \includegraphics[width=0.6\textwidth,keepaspectratio]{../SimpleResponsesChapter/gfx/PaoliniCV.pdf}
  \caption[Regularity categorisation in chopper units]{Average regularity profiles in chopper units using 10 msec interval CV measures at 30 dB relative to acoustic threshold. Data represents mean and standard deviation in units in each category. Data reproduced from Figure 2 in \citet{PaoliniClareyEtAl:2005}.}
...
#+BEGIN_LaTeX
  \begin{figure}[t!]
    \centering%
    \subfloat[Chopper Sustained]{\includegraphics[keepaspectratio,width=0.3\textwidth]{../SimpleResponsesChapter/gfx/TStellate/CS-01-864-004}}\hfill%\quad%
    \subfloat[Chopper Transient 1]{\includegraphics[keepaspectratio,width=0.3\textwidth]{../SimpleResponsesChapter/gfx/TStellate/CT1-01-857-007}}\hfill%\\
    \subfloat[Chopper Transient 2]{\includegraphics[keepaspectratio,width=0.3\textwidth]{../SimpleResponsesChapter/gfx/TStellate/CT2-01-305-014}}%\quad%
...
was 21 \um, representative of the average TS neuron in cats \citep{ReddCahillEtAl:2002}.
...
cell model subtypes. The weights (\wLSRTV, \wHSRTV) of \ANF synapses were used
in the optimisation procedures to allow for fine adjustments to the strength of
excitation. The number of synapses (\nLSRTS=30, \nHSRTS=30, \nDSTS=20,
\nTVTS=20, \nGLGTS=20), connectivity spread and delays were fixed at appropriate
values.
...
of a 50 msec \CF tone (5 msec on-off ramp) and analysed for \PSTH and
inter-spike interval characteristics
...
the \CV calculations. Additional penalties were added to the cost function
score if certain CV conditions weren't met. In all chopper units, if the CV
value in the first 10 msec period was above 0.2 a penalty of 10 times the
difference was applied. In the \ChS model, if the CV value at any time period
was above 0.3 a penalty of 100 times the difference was applied. In the \ChTone
model optimisation, for periods between 10 and 40 msec if the CV value was above
0.3 a penalty of 10 times the difference was applied. In the \ChTtwo model
optimisation, for periods between 10 and 40 msec if the CV value was below 0.3
or above 0.5 a flat 10 point penalty and a proportional penalty of 10 times the
...
#+BEGIN_LaTeX
  ...
  \end{equation} \noindent
#+END_LaTeX
where $\bar{\mathbf{v}}|_{t=0}^{t=5}$ is the average intracellular membrane voltage
between 0 and 5 ms. The onset ratio \AIV statistic is unitless.


The adaptation shift measurement calculates the drop in \AIV depolarisation at
the end of the stimulus compared to the stable point around 20 msec after the
onset
#+BEGIN_LaTeX
  \begin{equation}
  \label{eq:IVstats-Adap} \textrm{Adaptation} = \bar{\mathbf{v}}|_{t=18}^{t=23}\quad - \quad\bar{\mathbf{v}}|_{t=43}^{t=48}, \quad (mV).
  \end{equation}
#+END_LaTeX
...
paper \citep{PaoliniClareyEtAl:2005}. The second adaptation measure was shifted
from around 50 msec to 45 msec to avoid rapid \AIV drop-off in some units with a
delayed onset peak that shifted the AIV trace.
...
#+BEGIN_LaTeX
  \begin{equation}
  \label{eq:IVstats-Offset} \textrm{Offset} = \bar{\mathbf{v}}|_{t=-10}^{t=0}\quad - \quad\bar{\mathbf{v}}|_{t=60}^{t=65}, \quad (mV).
  \end{equation}
#+END_LaTeX
...
\DR or in the saturation region. The \CF of the simulated TS cell model was set
to 9.1 kHz (mean \CF of chopper units in \citet{PaoliniClareyEtAl:2005}) to
normalise properties of the TS cell models to improve comparison of optimised
parameters.
...
#+name: CV_RMS
#+BEGIN_SRC octave :var CH="CT2"  :export none :results replace raw
...
#+BEGIN_SRC octave :var CH="CS"  :export none :results replace raw
 ...
 sprintf ("%.3f  %.3f %.3f %.3f %.3f  %.3f %.3f %.3f",\
  mean(dat(:,2)),mean(dat(:,3)),\
mean (dat(1:4,2)),mean(dat(1:4,3)),\
mean (dat(5:8,2)),mean(dat(5:8,3)),\
mean (dat(9:12,2)),mean(dat(9:12,3)))
 % Relative error
 % RE= mean(abs((dat(1:end,2) - dat(1:end,3))./(dat(1:end,3))));
#+END_SRC

#+RESULTS: CV_mean_ChS
0.185  0.160 0.157 0.145 0.192  0.172 0.206 0.162
0.026  0.013 0.020 0.044
...
#+BEGIN_SRC octave :var CH="CS"  :export none :results replace raw
 ...
 dat = load('/tmp/CV_RMS.txt');
 sprintf ("%.3f  %.3f %.3f %.3f",\
  sqrt(meansq (dat(:,2)-dat(:,3))),\
sqrt(meansq (dat(1:4,2)-dat(1:4,3))),\
sqrt(meansq (dat(5:8,2)-dat(5:8,3))),\
sqrt(meansq (dat(9:12,2)-dat(9:12,3))))
 % Relative error
 % RE= mean(abs((dat(1:end,2) - dat(1:end,3))./(dat(1:end,3))));
#+END_SRC

#+RESULTS: CV_RMS_ChS
0.034  0.027 0.021 0.047


# call _ grabTSdata[ : in-file ./gfx/TStellate2_CS/TStellate.Fit.dat ](regexp="#  Input Resistance =",ind=5)[:results replace raw]

# + name: grabTSdata
# + BEGIN_SRC awk :in-file ./gfx/TStellate2_CS/TStellate.Fit.dat  :export none :var regexp="#  Input Resistance =" :var ind="5"
#  /$regexp/ {print $$ind}
# + END_SRC
...
pure \CF tones at 20, 30 and 40 dB re $\theta$ (above the acoustic threshold 20 dB
\SPL) of the exemplar unit (40, 50 and 60 dB \SPL, respectively). A 20 msec
delay was included to allow the model to reach a sustained \RMP. The \TS cell
model used in the \ChS optimisation, hereafter called the \ChS model, used the
Type I-t \RM neural model.
...
call_opt_paramf(param="w.x[hsr][ts]",type="CS",factor="1000")[:results replace raw] nS) and very low
...
call_CS_rate(SPLreTh=40)[:results replace raw], call_CS_rate(SPLreTh=50)[:results replace raw]
 and call_CS_rate(SPLreTh=60)[:results replace raw]
...
error metrics and additional penalties were applied when the model results were
outside acceptable ranges. The best fit model received no additional penalties.
The \RMS error between the CV data of the optimised \ChS model and the exemplar
\ChS unit (Figure \ref{fig:CSresults}B) was 0.034 combined (20dB: 0.027, 30dB:
0.021, 40dB: 0.047). In relative terms the mean CV at 30 dB re $\theta$ was 0.192
and 0.172 for the modelled and exemplar \ChS units, respectively. Figure
\ref{fig:CSresults}D shows the AIV measures used in the optimisation. The onset
ratio and the adaptation \AIV difference were only measured for the 30 dB re $\theta$
due to the limited availability of experimental data. The \ChS model's onset
ratio, 1.0125, was smaller than the exemplar unit's, 1.0417. The onset ratio was
affected by the synchronisation of AP peaks and troughs at onset that cancelled
each other out in the averaging of the membrane voltage. The adaptation AIV
difference was 1.67 mV for the model and 0.94 mV for the exemplar unit. The
drop-off in the experimental unit's \AIV (Figure \ref{fig:CSresults}C) at the
end of the stimulus affected the adaption measure by increasing the apparent
adaption drop in the \AIV. The post stimulus offset in the experimental \ChS
unit was below 3 mV for each of the stimuli. The \ChS model's post-stimulus
offset hyperpolarisation was consistently above 5 mV. This was due to
spontaneous spikes preceding the stimulus (increasing the apparent \RMP) and the
strong hyperpolarising dip due to inhibition and post-stimulus recovery of \HSR
units producing reduced excitatory input to the model.
...
#+BEGIN_LaTeX
  ...
  \caption[Transient chopper type 1 optimisation results]{Transient chopper type 1
    ...
    The RMP for the reference unit and best fit \ChTone model was -55.98 and -64.97
    mV, respectively. See Figure \ref{fig:CSresults} for figure configuration.
    A. PSTH of the optimised \ChTone  model at 30 dB $\theta$ (50 repetitions, CF 9.1 kHz, 50 dB SPL).
...
along with the experimental data for the \ChTone subtype
\citep[exemplar~unit~01-857-007, \CF 8.2~kHz][]{PaoliniClareyEtAl:2005}. The depolarisation
threshold for the exemplar unit was 55 dB \SPL, close to the average 47.2 dB
\SPL for all \ChTone units \citep{PaoliniClareyEtAl:2005}. The threshold for
the simulated model was 20 dB SPL so that the three stimuli to accommodate
comparisons with the other chopper models and for consistency in the inputs to
the model.
...
call_opt_paramf(param="w.x[glg][ts]",type="CT1",factor="1000")[:results replace raw] nS), but the longer \GABAa time constant would cause reasonable inhibition to
...
optimised \ChTone model. The PSTH of the model at 30 dB re $\theta$ (50 dB \SPL) is
shown in Figure \ref{fig:CT1results}A. The mean rates for the optimised \ChT1
model at 20, 30 and 40 dB re $\theta$ were
call_CT1_rate(SPLreTh=40)[:results replace raw],
call_CT1_rate(SPLreTh=50)[:results replace raw] and
call_CT1_rate(SPLreTh=60)[:results replace raw] spikes per second, respectively.
...
The combined \RMS error of the \CV data was 0.064. The CV at 30 dB re $\theta$ was
closest fit to the reference data (RMS error 0.034). The RMS error of the 20 and 40 dB
re $\theta$ CV profiles was 0.038 and 0.098, respectively. The \AIV traces in
...
\ChTone\space \AIV trace resulted in consistent membrane voltage fluctuation that was
...
the exemplar unit at each of the stimulus levels. The post-offset
hyperpolarisation results of the exemplar \ChTone unit were not consistent with
the inhibitory behaviour of \ChTone units. The lack of
hyperpolarisation in experimental unit recordings can be explained by one or more of
(1) the position of the electrode relative to the site of AP generation, (2) the distance to
inhibitory synapses on dendrites, and (3) shunting of excitatory currents
from distal inhibitory synapses.
...
experimental stimuli (25, 35 and 45 dB \SPL) were lower relative to those used
in the \ChT1 model optimisation routine (40, 50 and 60 dB \SPL). The main
difference between the transient chopper types is their \CV values after 10
msec, with the \ChT2 type becoming irregular (i.e. having a CV value over 0.3)
and maintaining that irregularity for the remainder of the stimulus. \ChT2 has
a higher reduction in onset depolarisation (90.2\%) compared to ChT1, and
...
The characteristic CV response of the \ChTtwo unit was closely matched at 30 and
40 dB re $\theta$ (Figure \ref{fig:CT2results}B). The mean rates for the optimised
\ChTtwo unit at the three \SPL tones were call_CT2_rate(SPLreTh=40)[:results replace raw],
call_CT2_rate(SPLreTh=50)[:results replace raw] and
call_CT2_rate(SPLreTh=60)[:results replace raw] spikes per second, respectively.

Synaptic activity from GLG, TV and DS cell model inputs reduced the regularity
of the spike intervals after 10 msec and increased CV throughout the stimulus.
The \RMP for the experimental and model \ChTtwo units was $-60.63$ and $-64.96$
mV, respectively. The experimental \ChTtwo unit had a greater reduction in
onset depolarisation (90.2\%) compared to \ChTone units at 30 dB re $\theta$
\citep{PaoliniClareyEtAl:2005}. The adaptation statistic results of the
optimised \ChTtwo model did not demonstrate a similar drop (around 6 mV) to that
shown in the reference data Figure \ref{fig:CT2results}D, despite reduction in
firing rate and strong changes in CV. The post-tone inhibitory offset of the
experimental \ChTtwo unit was the greatest of all the chopper subtypes (8.82 mV
at 40 dB re $\theta$). The optimised \ChTtwo unit had a stable post-offset inhibition
around 4 mV at the three stimulus levels.

Excitatory synaptic weights (\wLSRTS
call_opt_paramf(param="w.x[lsr][ts]",type="CT2",factor="1000")[:results replace raw] nS,
\wHSRTS
...
but the loud input stimulus (60 dB SPL) would cause
...
** Verification of the TS Cell Models


Figures \ref{fig:CSVerificationRL}, \ref{fig:CT1VerificationRL} and
\ref{fig:CT2VerificationRL} show the responses of the \TS cell models across the
nuclei to the verification stimuli, \CF tones in the central channel and
broadband noise.
# The major difference between the optimised routine parameters
# and the CNSM model used in the verification is the species (rat for
# optimisation, cat for verification).

The \ChS model's response area to tones, Figure \ref{fig:CSVerificationRL}A,
show a narrow receptive field with a threshold at 10 dB SPL and a \Qten of 0.143
octaves (three active channels at 20 dB SPL). The on-CF unit had a large dynamic
range to tones ($>$60 dB, Figure \ref{fig:CSVerificationRL}C) due to similar
strength of synaptic inputs from LSR and HSR fibres. The \ChS model's response
to noise, Figures \ref{fig:CSVerificationRL}B and D, were monotonic with
acoustic threshold dependent on HSR inputs.

#+BEGIN_LaTeX
  \begin{figure}[t!]
  \centering
  \resizebox{0.95\textwidth}{!}{\includegraphics{../SimpleResponsesChapter/gfx/Responses/ratelevel-0-ChS.pdf}}\\
  \caption[Whole network response of ChS TS cell model]{Response of the ChS subtype TS cell model across the
    whole network to a pure tone (on centre unit's CF, 4.513 kHz) and broadband noise.
    A. Rate level response map of the TS cells to a pure CF tone (4.5 kHz, 50 msec, 100 repetitions).
    B. Noise rate level response map of the cells to broad-band noise (50 msec, 100 repetitions).
    C. Rate level responses of the central unit to a pure CF tone.
    D. Noise rate level response of the central unit to BNN. }
  \label{fig:CSVerificationRL}
  \end{figure}
#+END_LaTeX

The \ChTone model's response area to tones, Figure \ref{fig:CT1VerificationRL}A,
show a narrow receptive field with a threshold and \Qten similar to the \ChS
model. The on-CF unit had a small dynamic range (around 20 dB, Figure
\ref{fig:CT1VerificationRL}C), saturating around 30 dB with rate just under 250
sp/s. The unit also showed slight depression of the rate-level curve at high
sound levels, likely due to inhibitory influences. In response to noise, the
\ChTone model's threshold increased and its saturation rate decreased to just
over 200 sp/s (Figures \ref{fig:CSVerificationRL}B and D).


#+BEGIN_LaTeX
  \begin{figure}[t!]
  \centering
  \resizebox{0.95\textwidth}{!}{\includegraphics{../SimpleResponsesChapter/gfx/Responses/ratelevel-0-ChT1.pdf}}\\
  \caption[Whole network response of ChT1 TS cell model]{Response of the \ChTone subtype \TS cell model across the
    whole network to a pure tone (on centre unit's CF, 4.513 kHz) and broadband noise.
    A. Rate level response map of the TS cells to a pure CF tone (4.5 kHz, 50 msec, 100 repetitions).
    B. Noise rate level response map of the cells to broad-band noise (50 msec, 100 repetitions).
    C. Rate level responses of the central unit to a pure CF tone.
    D. Noise rate level response of the central unit to BNN. }
  \label{fig:CT1VerificationRL}
  \end{figure}
#+END_LaTeX

The \ChTtwo model's response area to tones and noise is shown in Figures
\ref{fig:CT2VerificationRL}A and B. The tuning of the tone response area is
similar to the \ChS and \ChTone models but with higher firing rates. The on-CF
\ChTtwo unit had a high spontaneous rate ($>$100 sp/s) and a high maximum firing
rate (over 350 sp/s) at 30 dB SPL Its tone rate level curve (Figure
\ref{fig:CT2VerificationRL}C) had a small dynamic range (around 20 dB) and
immediate linear depression. In response to noise, the \ChTone model's had a
compressed rate-level curve (Figure \ref{fig:CSVerificationRL}D) with increased
threshold similar to the other models, reduction in its maximum firing rate and
reduction of rate at high SPL similar to the \ChTone model.


#+BEGIN_LaTeX
  \begin{figure}[t!]
  \centering
  \resizebox{0.95\textwidth}{!}{\includegraphics{../SimpleResponsesChapter/gfx/Responses/ratelevel-0-ChT2.pdf}}\\
  \caption[Whole network response of ChT2 TS cell model]{Response of the \ChTtwo subtype TS cell model across the
    whole network to a pure tone (on centre unit's CF, 4.513 kHz) and broadband noise.
    A. Rate level response map of the TS cells to a pure CF tone (4.5 kHz, 50 msec, 100 repetitions).
    B. Noise rate level response map of the cells to broad-band noise (50 msec, 100 repetitions).
    C. Rate level responses of the central unit to a pure CF tone.
    D. Noise rate level response of the central unit to BNN. }
  \label{fig:CT2VerificationRL}
  \end{figure}
#+END_LaTeX
...
the main projection neurons, \TS cells. The diversification of the physiological
...
The \ChTone model had an RMS error of 0.074 at 30 dB re $\theta$, which was
stimulated at a lower level than the experimental unit (50 dB SPL against 85 dB
SPL), which may have had more active inputs from the other cells in the \CNSM
model. The CV data of the \ChTtwo model satisfied the expected responses at 20
and 30 dB re $\theta$, but failed to generate regular onset spikes at 40 dB re $\theta$.
# \note{If the ChT2 model changes change the previous statement}
Along with stringent penalties to constrain the CV response output in the TS cell optimisation procedure, the CV measure
was an effective tool in the optimisation of the TS cell model.
...
The expected output of gradual increase in the strength of inhibition from \ChS
to \ChTone to \ChTtwo is only present in \GLG cell inputs to the TS cell models.
\DS cell inhibition is strongest in each of the TS models and the weights of TV
cell inputs are considerably lower. The \SPL of the three \CF tones used in
each optimisation was set based on an acoustic threshold of 10 dB SPL that
differed from the exemplar units'. In this case the input inhibitory cells
(GLG, DS and TV cells) were all moderately active and may have differed from the
range of possible input rates and dynamics of the experimental data.
...
** Optimised Parameters of the CNSM Model


Table \ref{tab:Finalparams} show the aggregated optimised parameter values of
the synaptic strength in the CNSM model. One important point for the table is
that the synaptic weights of \LSR fibres in the CNSM model appears to have a
stronger value than \HSR\space \ANFs in each of the \CN units. Their large
dynamic range was beneficial to the GLG and DS cell models that were fitted to
experimental data with monotonic rate level responses. For TS and TV cell
models, LSR ANFs are the only excitatory source that counters the increasing
inhibitory sources, GLG and DS cells, at high SPL. TV cells have a particularly
high level of DS and GLG cell inhibition compared to the different TS cells
models. The expected output of gradual increase in the strength of inhibition
from \ChS to \ChTone to \ChTtwo is only present in \GLG cell inputs to the TS
cell models. \DS cell inhibition is strongest in each of the TS models and the
weights of TV cell inputs are considerably lower.

The number of connection, connectivity spread and delay parameters were assigned
values based on evidential and physiologically plausible guesses. The
exceptions were the number of connections in the \GLG cell optimisation and the
\DS cell rate level optimisation. These assumptions are necessary to remove the
burden of coarse granular steps so that the optimsation can focus on the
fine-grained detail of synaptic weights. Further investigation of optimisation
processes and routines will help understand the neural network dynamics in the
stellate microcircuit and help to identify its key components.

#+begin_src latex :exports results :results latex  :noweb yes
  \begin{table}[htb!]
    \centering
    \footnotesize
    \caption[Optimised synaptic weight and number of connections in the CNSM model]{Optimised synaptic weight and number of connections in the CNSM model. The units of synaptic weight are in $10^{-9}$ Seimens with the exception of GLG cell inputs that are unitless.}
    \label{tab:Finalparams}
    <<booktabz(table=finalparams,align="lXXXXXX",env="tabularx",width="\\textwidth")>>
  \end{table}
#+end_src

# # Delete all source block results -- incomplete, waiting on
...
#+name: finalparams
#+BEGIN_SRC octave :export none :results value vector
  clear
  [sysstat out] = system ("  grep -A6 -e '^# weights' ../SimpleResponsesChapter/gfx/TStellate2_CS/TStellate.Fit.dat | tail -5 | tr -d '#'");
  wchs = str2num(out);
  [sysstat out] = system ("  grep -A6 -e '^# weights' ../SimpleResponsesChapter/gfx/TStellate2_CT1/TStellate.Fit.dat | tail -5 | tr -d '#'");
  wcht1 = str2num(out);
  [sysstat out] = system ("  grep -A6 -e '^# weights' ../SimpleResponsesChapter/gfx/TStellate2_CT2/TStellate.Fit.dat | tail -5 | tr -d '#'");
  wcht2 = str2num(out);
  [sysstat out] = system ("  grep -A6 -e '^# num. connections' ../SimpleResponsesChapter/gfx/TStellate2_CS/TStellate.Fit.dat | tail -5 | tr -d '#'");
  nchs = str2num(out);
  [sysstat out] = system ("  grep -A6 -e '^# num. connections' ../SimpleResponsesChapter/gfx/TStellate2_CT1/TStellate.Fit.dat | tail -5 | tr -d '#'");
  ncht1 = str2num(out);
  [sysstat out] = system ("  grep -A6 -e '^# num. connections' ../SimpleResponsesChapter/gfx/TStellate2_CT2/TStellate.Fit.dat | tail -5 | tr -d '#'");
  ncht2 = str2num(out);
  ...
  wtable=zeros(5,6);                          ntable=zeros(5,6);
  wtable(:,1) = wchs([4 5 3 2 1],4);          ntable(:,1) = nchs([4 5 3 2 1],4);
  wtable(:,2) = wchs([4 5 3 2 1],3)*1000.0;   ntable(:,2) = nchs([4 5 3 2 1],3);
  wtable(:,3) = wchs([4 5 3 2 1],2)*1000.0;   ntable(:,3) = nchs([4 5 3 2 1],2);
  wtable(:,4) = wchs([4 5 3 2 1],1)*1000.0;   ntable(:,4) = nchs([4 5 3 2 1],1);
  wtable(:,5) = wcht1([4 5 3 2 1],1)*1000.0;  ntable(:,5) = ncht1([4 5 3 2 1],1);
  wtable(:,6) = wcht2([4 5 3 2 1],1)*1000.0;  ntable(:,6) = ncht2([4 5 3 2 1],1);

  ## FIX GLG INPUTS - GLG neural model does not use N
  ntable(1,1)=1;
  ntable(2,1)=1;
  ...
  ##  HEADER
  ## s=""; #
  s="  \t\\GLG\t\\DS\t\\TV\t\\ChS\t\\ChTone\t\\ChTtwo \n--\t\t\t\t\t\t----\n"
  ## BODY
  ## stmp="\\HSR\t";  for i=1:6
  ##   stmp=sprintf("%s%.4g--%d \t",stmp,wtable(1,i),ntable(1,i)); end; s = sprintf("%s%s\n",s,stmp);
  ## s = [s "LSR\t" sprintf("%.4g\t",wtable(2,:))]; s = [s sprintf("\n")];
  ## s = [s "GLG\t" sprintf("%.4g\t",wtable(3,:))]; s = [s sprintf("\n")];
  ## s = [s "DS\t" sprintf("%.4g\t",wtable(4,:))]; s = [s sprintf("\n")];
  ## s = [s "TV\t" sprintf("%.4g\t",wtable(5,:))];

  ## stmp="\\LSR\t"; for i=1:6  stmp=sprintf("%s%.4g--%d \t",stmp,wtable(2,i),ntable(2,i));end; s = sprintf("%s%s\n",s,stmp);
  ## stmp="\\GLG\t"; for i=1:6 stmp=sprintf("%s%.4g--%d \t",stmp,wtable(3,i),ntable(3,i));end; s = sprintf("%s%s\n",s,stmp);
  ## stmp="\\DS\t"; for i=1:6  stmp=sprintf("%s%.4g--%d \t",stmp,wtable(4,i),ntable(4,i));end; s = sprintf("%s%s\n",s,stmp);
  ## stmp="\\TV\t"; for i=1:6   stmp=sprintf("%s%.4g--%d \t",stmp,wtable(5,i),ntable(5,i));end; s = sprintf("%s%s",s,stmp);

  stmp =sprintf("%.4g$\\,\\times\\,$%d \t",[wtable(1,:)' ntable(1,:)']'); s = sprintf("%s\\HSR\t %s\n",s,stmp);
  stmp =sprintf("%.4g$\\,\\times\\,$%d \t",[wtable(2,:)' ntable(2,:)']'); s = sprintf("%s\\LSR\t %s\n",s,stmp);
  stmp =sprintf("%.3g$\\,\\times\\,$%d \t",[wtable(3,:)' ntable(3,:)']'); s = sprintf("%s\\GLG\t %s\n",s,stmp);
  stmp =sprintf("%.4g$\\,\\times\\,$%d \t",[wtable(4,:)' ntable(4,:)']'); s = sprintf("%s\\DS\t %s\n",s,stmp);
  stmp =sprintf("%.4g$\\,\\times\\,$%d \t",[wtable(5,:)' ntable(5,:)']'); s = sprintf("%s\\TV\t %s",s,stmp);
  sprintf("%s",s)
#+END_SRC


#+tblname: finalparamshead
|   | \GLG | \DS | \TV | \ChS | \ChTone | \ChTtwo |
...
# + BEGIN_SRC sh :export none
#  grep -A6 -e '^# weights' ../SimpleResponsesChapter/gfx/TStellate2_CT2/TStellate.Fit.dat | tail -5 | tr -d '#'
# + END_SRC


# + call: booktabs (table=bestopt,align="cXXXXXX",env="tabularx")[ :results latex :exports results ]
...
# call _ opt_paramf(param="w.x[lsr][ts]",type="CS",factor="1000")[:results replace raw]


#+tblname: months :export none
| num | Abbrev. |
|-----+---------|
|   1 | Jan.    |
|   2 | Feb.    |
...
#+tblname:   optnotes
| \wLSRTS | call_opt_paramf(param="w.x[lsr][ts]",type="CS",factor="1000")[:results replace raw] | call_opt_paramf(param="w.x[lsr][ts]",type="CT1",factor="1000")[:results replace raw] | call_opt_paramf(param="w.x[lsr][ts]",type="CT2",factor="1000")[:results replace raw] |

| \wHSRTS | call_opt_paramf(param="w.x[hsr][ts]",type="CS",factor="1000")[:results replace raw] | call_opt_paramf(param="w.x[hsr][ts]",type="CT1",factor="1000")[:results replace raw] | call_opt_paramf(param="w.x[hsr][ts]",type="CT2",factor="1000")[:results replace raw] |
|         |                                                                                    |                                                                                     |                                                                                     |
...
# |\multicolumn{7}{l}{ call_opt_paramf(param="w.x[lsr][ts]",type="CS",factor="1000")[:results replace raw] }      |

#+CAPTION: Mockup for possible parameter table with fixed or optimised parameters
#+label:   fig:Mockup
|        | \multicolumn{5}{c}{Source Cell type} |            |                   |              |            |
| Target | \LSR                                 | \HSR       | \GLG              | \DS          | \TV        |
|--------+--------------------------------------+------------+-------------------+--------------+------------|
| \GLG   | [d](s,w)                             | [s,d](w)   | (-)               | (-)          | (-)        |
...
available at the time of the model's development. The fractional Gaussian noise
component in the IHC-AN synapse was removed to ensure consistence in the \SR of
the \ANF models. Future work in this field may benefit from using the complete
\citet{ZilanyBruceEtAl:2009} AN model and any future improvements.
