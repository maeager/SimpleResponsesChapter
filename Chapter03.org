...
T he neural models used in the \ANFs and Golgi cell model were
...
The CV responses of the optimised model were
...
The \FSL at 30 dB re threshold (call_CT2_fsl(SPLreTh=40)[:results replace raw] ms) showed a precise first spike but the following spikes were too irregular for this unit to be classified a chopper (CV$_{0-10} > 0.3$).
