...
#+BEGIN_LaTeX
      \begin{table}[tp]
        ...
        \begin{tabularx}{0.8\linewidth}{lXXXXX}\toprule
                        & \HSR \ANF &            \TS             &            \DS             &            \TV             & Golgi \\ \midrule
...
#+BEGIN_LaTeX
  ...
  \begin{table}[htb]
  %  \centering 
  \begin{tabularx}{\textwidth}{|X|c|c|r@{.}|}\hline
...
#+BEGIN_LaTeX
  ...
  \begin{tabular}{Xcc@{,}r@{.}}
            \textbf{Parameters}           &    \textbf{Name}    & \textbf{Range} & \textbf{Best Values} \\\hline
        Weight of Golgi on \DS (nS)        &       \wGLGDS       &   [0.01,50]    & 0.532 \\ \hline
...
#+BEGIN_LaTeX
  ...
  \begin{table}[htb]
    ...
    \label{tab:DSRateLevelResults}
        \begin{tabularx}{0.48\textwidth}{|X|c|r@{.}|} \toprule%    \hdr{2}{}{DS\ Cell Optimisation: Best Fit Parameters } \\ 
      \textbf{Parameters}  &\textbf{Range}& \textbf{Final Value} \\ \midrule
    \wGLGDS  \hfill (nS)  & [0.01,50]   &10.1869   \\         
    \wHSRDS  \hfill (nS) &  &0.167221\ \\         
    \wLSRDS  \hfill (nS)  & & 4.83773  \\         
          \nLSRDS        & & 56.6  (57)   \\             
          \nHSRDS        & & 157.8  (158)   \\
    \nGLGDS      &  & 8.9  (9)   \\ \midrule
       Tone RL\ Error (sp/s)   &    25.62\ &\\   
...
#+BEGIN_LaTeX
  ...
  \begin{table}[htbp]
    ...
    \begin{tabularx}{Xc@{,}r{.}} \toprule
  \textbf{Parameters}& \textbf{Ranges}&\textbf{Final Value} \\ \otoprule
          \wLSRTV  \hfill (nS)    &     & 4.765  \\ 
          \wHSRTV  \hfill (nS)    &     & 0.983 \\ 
          \wDSTV  \hfill (nS)      &    & 1.152 \\ 
                \nDSTV  \hfill ()   &           & 29.4917 \\
       \TV \Erev     \hfill (mV)     &  & -68.25 \\ \midrule
        RL error  \hfill (spikes/s)      & & 36.75 \\
  Without DS RL error  \hfill (spikes/s) & & 11.33 \\
       NRL error  \hfill (spikes/s)      & & 13.87 \\
  Without DS NRL error  \hfill (spikes/s)& & 67.42 \\
...
\yellownote{caption these tables and refer to them}
# 50 dB Run
#+BEGIN_LaTeX
  {\small%
    \noindent%
...
#+BEGIN_LaTeX
  ...
  \caption{Final network parameters of DS to TV cell connections using the notch optimisation.}
   \label{tab:TVDSfinal} 
     \begin{tabularx}{\linewidth}{X c c@{,} r@{.}} \toprule %\hdr{4}{}{ TV Cell model Optimisation} \\ \hline 
                  \textbf{Parameters}                  &      \textbf{Name}      & \textbf{Range}& \textbf{Best Values} \\ \otoprule
...
#+BEGIN_LaTeX
  ...
  \begin{tabularx}{\linewidth}{Xccc@{.}c@{.}c@{.}} \toprule  %\hdr{6}{F}{Optimisation} \\ \hline
  ...
  Maximum leak conductance in TS cell  & \gleak \hfill (mS cm$^{-2}$) &    [1e-3,3]    & 0.657345  & 0.494097  & 0.72474\\
   Maximum KHT conductance in TS cell  & \gKHT  \hfill (S cm$^{-2}$)  &  [1e-6,0.003]  & 0.0189416 & 0.018941  & 0.022858\\
      %      TS reversal potential     &     \Eleak  \hfill (mV)      & [$-45$,$-70$]  &           &  -54.26   & -63.502\\ \hline
...
evidence that describes the behaviour of each interneuron in the network.  The
