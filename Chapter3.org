...
following cells and models:
  1. The auditory model: The Carney phenomenological auditory model
     ...
     from an arbitrary input stimulus.  The base line in Figure
  ...
  2. The Golgi cell: a \GABA-ergic \VCN marginal shell unit that is
     presumed to regulate excitability in the \GCD and core \VCN
     units \citep{FerragamoGoldingEtAl:1998}.  Only one /in vivo/ study
     has recorded extracellular data in the marginal shell area of the \CN
  ...
  3. The D stellate cell: a glycinergic, large multipolar cell with
     \OnC \PSTH response that acts as a coincidence detector.  Its
  ...
  4. The Tuberculoventral cell: a glycinergic, type II \EIRA unit in
     the deep layer of the \DCN \citep{SpirouDavisEtAl:1999}.  This cell
...
can be defined by:
  ...
  - $\mathbf{n}_{\textrm{{Pre}}\to\textrm{{Post}}}\xspace$ is the number of presynaptic
  ...
  - $\mathbf{s}_{\textrm{{Pre}}\to\textrm{{Post}}}\xspace$ is the spatial or feature
     ...
     offset.
  - $\mathbf{o}_{\textrm{{Pre}}\to\textrm{{Post}}}\xspace$ is the offset in distribution
    ...
    $i$.
  - $\mathbf{d}_{\textrm{{Pre}}\to\textrm{{Post}}}\xspace$ is the temporal delay between
...
Golgi cells down to four major details:
 1. Golgi cells are classic repetitively-firing neurons due to their type
    I current clamp response \citep{FerragamoGoldingEtAl:1998},
 ...
 3. The low threshold in Golgi cells, \citet{GhoshalKim:1997}, can\-not be due
    ...
    regarding type II \ANF units, that do project to the \GCD, and granule
    cell response to acoustic input meant that a Poisson rate neural model would
    be preferred over the Hodgkin-Huxley type neural model.  Although \HSR
    \ANF terminals do not generally project into the \GCD, they are
 ...
 4. the minimum \EPSP to shock of the AN
    ...
    stimuli \citep{GhoshalKim:1997} are significantly different from the
    core \VCN units.
...
\HSR and \LSR \ANF inputs to Golgi cells were determined the
Gaussian distribution in units of channel separation in the network.  The
weighted sum of \HSR and \LSR instantaneous-rate vectors are
...
but the bandwidth is difficult to ascertain.  The dendrites of D stellate
...
200 microns in the \GCD and \ANF tonotopic organisation in the
\GCD is less defined.  The \GLGDS spread is set to 2 channels with
zero offset, which corresponds to a \DS cell selecting from approximately
...
(\dANFDS).  The first spike latency in high \CF \DS cells ($2.8 \pm 0.09$
...
- \textit{(From Hancock Davis Voigt 97) Blum
...
\TV cell's leak reversal potential \Eleak scaling down to -75
mV\@.
...
generated in GNU Octave using Chebychev II
filters
...
the =libresample= interface to the locally compiled NEURON executable
/special/.
