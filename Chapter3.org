...
#+caption: [Basilar membrane frequency-distance function parameters]{Frequency to basilar membrane distance function parameters \citep{FitzGeraldBurkittEtAl:2001}. Data obtained from \url{http://earlab.bu.edu}.
#+label: tab:f2x
|       |      A |    a |         k |    L |
...
\begin{equation}
  \label{eq:delay}
   delay(x) = A_0 \times {\exp}(-x / A_1 ) \times 10^{-3} - 1.0/CF
\end{equation}
...
\citep{ZilanyBruceEtAl:2009}: \(delay(x) = A_0 \times {\exp}(-x / A_1) \times 10^{-3} \)
where $A_0 = 3.0$, $A_1 = 12.5$. In humans, the delay function is: \( delay =
4.915 + 0.3631 \times \exp(0.11324\timesx),\quad 5<x<35 \quad (\mathrm{mm}) \).
