...
#+CAPTION: [Experimental data of a single Type-II\space DCN\space unit]{Experimental data of a single Type-II\space DCN\space unit \citep[Fig.~1]{SpirouDavisEtAl:1999}. \yellownote{Figure \ref{fig:SpirouFig1} needs permission}}
...
#+CAPTION: [TV model optimisation configuration]{Expected mean rate response to  notch noise in the TV cells is created from 1 octave notch sweeps (top) for  the falling edge and from half octave notch sweeps (bottom) for the rising  edge. (Top and bottom figures from  \citealt[Fig.~9]{ReissYoung:2005})}
#+LABEL: fig:TVNotchDiagram
  [[file:./gfx/TV_NotchDataConfig]]
...
#+LABEL: fig:PaoliniCVdata
  [[file:../../cnstellate/TStellate/PaoliniCV.png]]
