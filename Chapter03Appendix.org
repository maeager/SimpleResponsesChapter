...
The \CNSM software code used in
Chapters 3 and 4 is called [[latex:progname][cnstellate]] and is implemented in [[latex:progname][NEURON]], [[latex:progname][NMODL]] and [[latex:progname][C]]
source code.

The \AN models that can be used by the CNSM model include version 6 \citep{ZilanyBruceEtAl:2009}
version 5 \citep{ZilanyBruce:2007,ZilanyBruce:2006},
version 4 \citep{TanCarney:2003},
version 3 \citep{HeinzZhangEtAl:2001}.  
The first run of a stimulus may take some time if the AN responses have
not been previously saved. The AN responses are
...
The directory for the saved AN responses is determined by the variable
[[latex:progname][=ANsoundspath]]=.
