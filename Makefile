...
#onlysources.svg	?=
#onlysources.eps.gz	?=
#onlysources.eps	?= ../figures/NoFigure.eps
#
...
#includes.svg		?=
#includes.eps.gz	?=
#includes.eps		?= ../figures/NoFigure.eps
#
...
rm_ext		:= \
	...
	fls vrb idx ind ilg acr acn alg glg glo gls lox nls nlo nlg brf mtc maf brf ist fmt
...
define run-latex
$(latex_build_program) -jobname='$1' -interaction=batchmode -file-line-error $(if $2,$2,) $1 > /dev/null
endef
...
%.gls:	%.glo %.tex %.ist
	$(QUIET)$(call echo-build,$<,$@)
	$(QUIET)$(call run-makeindex,$<,$@,$*.glg,$*.ist) #,-s nomencl.ist)
