...
defined.  The \GLGDS spread is set to 2 channels with zero offset, which corresponds to a \DS cell selecting from approximately 5 nearest Golgi cells.  
...
ms. Although the experimental stimuli was presented every 250 msec, the
...
PSTH was produced from the DS cells' spikes. Spike counts for 2 ms after the
...
The six parameters to be fit by the routine are the weights of \GLG, \HSR, and
...
responses (2,3,4 ms), hence the \DS~cell leak
%and \KLT~conductance parameters 
